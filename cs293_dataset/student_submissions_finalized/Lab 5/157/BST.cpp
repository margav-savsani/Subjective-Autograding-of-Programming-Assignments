...
bool BST::insert(int jrnyCode, int price)
{
  ...
  if(y!=nullptr) {
    if(BSTLessThan(newnode,y)) {y->left= *newnode; y->left->parent=y;} //if less than parent price insert at parent's left
    else {y->right= *newnode; y->right->parent=y;} //else insert at right
  }
  htupdt(newnode);
  return true;
}
...
void BST::htupdt(TreeNode *node1){
  if(node1->left==nullptr && node1->right==nullptr) node1->height=0;
  else if (node1->left==nullptr && node1->right!=nullptr) node1->height=node1->right->height+1;
  else if (node1->left!=nullptr && node1->right==nullptr) node1->height=node1->left->height+1;
  else node1->height=max(node1->left->height,node1->right->height)+1;
  if(node1->parent!=nullptr) htupdt(node1->parent);
  return;
}
