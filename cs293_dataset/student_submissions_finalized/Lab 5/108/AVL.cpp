...
void AVLTree::rotateleft(TreeNode* node) {
    ...
    rgt->height = 1 + max(rgt->left ? rgt->left->height:0, rgt->right ? rgt->right->height:0);
}
...
bool AVLTree::insert(int jrnyCode, int price) {
    ...
    while (new_node) {
        new_node->height = 1 + max(new_node->left ? new_node->left->height:0, new_node->right ? new_node->right->height:0);
        int imbalance = (new_node->right ? new_node->right->height:0) - (new_node->left ? new_node->left->height:0);
        cout << imbalance << '\n';
        if (imbalance == 2) {
            if (new_node == root) root = new_node->right;
            rotateleft(new_node);
        } else if (imbalance == -2) {
            if (new_node == root) root = new_node->left;
