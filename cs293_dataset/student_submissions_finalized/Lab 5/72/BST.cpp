...
bool BST::insert(int jrnyCode, int price)
{
  ...
  //Checking whether the code is already in the tree and return false if it's already present
  if(find(jrnyCode))return false;
  // Implement insertion in BST
  TreeNode *z = new TreeNode(jrnyCode, price);
  if(root == nullptr){
    updateBSTParentChild(z,nullptr, 0);
  }
  else{
...
int BST::find(int jrnyCode)
{
  ...
  while( currNode != nullptr){
    ...
    if(!(v1 || v2))return 1;
    else if(v1)currNode = currNode->right;
