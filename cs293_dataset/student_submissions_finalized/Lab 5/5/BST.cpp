...
bool BST::insert(int jrnyCode, int price)
{
 ...
 TreeNode* node = new TreeNode(JrnyCode, price);
  ...
  if(BSTLessThan(node, parent))
  {
    updateBSTParentChild(node, parent ,true);    
  }
  else
  {
    updateBSTParentChild(node, parent, false);
  }
...
int BST::find(int jrnyCode)
{
  ...
  while(temp != NULL)
  {
    if(BSTLessThan(node , temp)) temp = temp->left;
    else if(BSTLessThan(temp, node))temp = temp->right;
    else return 1; // Means we found the Jrny code
  }
