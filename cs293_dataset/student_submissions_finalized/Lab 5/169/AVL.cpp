...
{TreeNode *x = root;
    TreeNode *y = x;
    if (root == NULL) {
    root->jrnyCode=jrnyCode;
    root->price=price;
    return true;
    }
    while(x!=nullptr){
    if (jrnyCode==root->jrnyCode)
    return false;
    if (jrnyCode-root->jrnyCode<0){
        y =x;
        if(y->right==nullptr){
...
bool b1=false;
bool b2=true;
while(b1 ==false && x != nullptr && b2==true){
if(x->price ==price){
b1=true;
break;
}
else if(x->price > price ){if( x->left != nullptr){x=x->left;}
else {
b2=false;
break;
}
}
else if(we->price < price){if(x->right != nullptr){
x=x->right;
}
else{
b2=false;
