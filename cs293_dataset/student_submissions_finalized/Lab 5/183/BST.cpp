...
bool BST::insert(int jrnyCode, int price)
{
  ...
  if(parentOfCurrNode->left==currNode){if ((parentOfCurrNode->right!=NULL)&&(parentOfCurrNode->right->height>parentOfCurrNode->left->height)){break;}
                                       else parentOfCurrNode->height++;
                                            parentOfCurrNode=parentOfCurrNode->parent;
                                            currNode=currNode->parent;}
   if(parentOfCurrNode->right==currNode){if ((parentOfCurrNode->left!=NULL)&&(parentOfCurrNode->left->height>parentOfCurrNode->right->height)){break;}
                                       else parentOfCurrNode->height++;
                                            parentOfCurrNode=parentOfCurrNode->parent;
                                            currNode=currNode->parent;}
  } 
...
bool BST::remove(int jrnyCode)
{
  ...
  if(find(jrnyCode)==false){return false;}
    ...
    while(x!=NULL){
              if (BSTLessThan(z,x)){x=x->left;}       //if JourneyCode is less then search in left subtree
             else if(BSTLessThan(x,z)){x=x->right;}
             else {return true;}
             }
    if (x->left == NULL || x->right == NULL) {
        ...
        if (prev == NULL){return newx;}
        if (x == prev->left){prev->left = newx;}
        else {prev->right = newx;}
        free(x);
    }
    else {
        TreeNode* p = NULL;
