...
{  
  ...
  struct TreeNode *x=parentOfCurrNode;
  ...
  while(true){
      if((((z->left->height)-(z->right->height))<-1)||(((z->left->height)-(z->right->height))>1)){break;}
      else { x=x->parent;y=y->parent;z=z->parent;}
      }
  if((z->right==y)&&(y->left==x)){rotateRightLeft(x,y,z);}
  if((z->right==y)&&(y->right==x)){rotateRightRight(x,y,z);}
  if((z->left==y)&&(y->left==x)){rotateLeftLeft(x,y,z);}
  if((z->left==y)&&(y->right==x)){rotateLeftRight(x,y,z);} 
  return true;
...
bool AVLTree::remove(int jrnyCode)
{
  ...
  if(find(jrnyCode)==false){return false;}
    ...
    while(x!=NULL){
              if (AVLLessThan(z,x)){x=x->left;}       //if JourneyCode is less then search in left subtree
             else if(AVLLessThan(x,z)){x=x->right;}
             else {return true;}
             }
