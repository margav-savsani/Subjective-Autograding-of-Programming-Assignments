...
public:
  ...
  //similar to above except it is used for equality comparisons
  bool operator = (TreeNode const &node)
  {
    if (JourneyCode == node.JourneyCode) return true;
    else return false;
  ...
  //function to access the price of node in case we need it as it is a private variable
  int givePrice(TreeNode const &node){
    return node.price;
  }
