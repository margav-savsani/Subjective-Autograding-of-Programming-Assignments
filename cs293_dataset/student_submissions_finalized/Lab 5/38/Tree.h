...
public:
    ...
    void insert(int JournetCode, int price)
    {
        TreeNode a(JournetCode, price);
        if (this->JourneyCode < JournetCode) 
        {
            if (left == nullptr) this->left = &a;
            else return left->insert(JournetCode, price);
        }
        else 
        {
            if (right == nullptr) this->right = &a;
            else return right->insert(JournetCode, price);
        }
    }
};
