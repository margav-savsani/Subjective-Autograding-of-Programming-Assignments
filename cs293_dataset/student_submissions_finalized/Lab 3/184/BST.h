...
struct TreeNode{
    ...
    bool insert_node(int code,int cost){
        if(JourneyCode==0){
                JourneyCode=code;
                price=cost;
                return true;
        }
        else if(JourneyCode==code){
                return false;
        }
        else{
                if(cost<price){
                        left=new TreeNode();
                        left->parent=left;
                        left->insert_node(code,cost);
                }
                else if(cost>price){
                        right=new TreeNode();
                        right->parent=right;
                        right->insert_node(code,cost);
                }
        }
    }

    bool find_node(int code,int cost){
        if(JourneyCode==0){
                return false;
        }
        else if(JourneyCode==code && price==cost){
                return true;
        }
        else{
                if(cost<price && left!=nullptr){
                        left->find_node(code,cost);
                }
                else if(cost<price && left==nullptr) return false;
                else if(cost>price && right!=nullptr){
                        right->find_node(code,cost);
                }
                else if(cost>price && right==nullptr){
                        right->find_node(code,cost);
                }
    ...
    bool remove_node(int code, int cost){

    }

    void traverse_node(int type){
