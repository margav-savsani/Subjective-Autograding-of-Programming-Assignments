...
bool BST::find(int JourneyCode, int price) { 
    ...
    node=root;
    int track=0;
    if(node->price == price && node->JourneyCode == JourneyCode){track=1;return true;}
    else {
        if(price>=node->price){
            node=node->left;
  ...
  TreeNode * node;
    ...
    if(node->price == price && node->JourneyCode == JourneyCode){
        if(node->right==nullptr && node->left==nullptr){
            node->JourneyCode=0;node->price=0;return true;
        }
        else if(node->right !=nullptr && node->right == nullptr){
            (node->right)->parent = node->parent;return true;
        }
        else if(node->right ==nullptr && node->right != nullptr){
            (node->left)->parent = node->parent;return true;
        }
        else{
            TreeNode *right = node->right;
            TreeNode *t = node->left->left;
            while(true){
