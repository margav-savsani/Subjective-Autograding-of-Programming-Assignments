...
public:
        ...
        bool findjourneycode(int JourneyCode, int price);
        ...
        struct TreeNode *constructBST(int preorder[], int journey[], int start, int end)
        {
                // base case
                if (start > end)
                {
                ...
                struct TreeNode *node = new TreeNode(journey[start], preorder[start]);
                ...
                node->left = constructBST(preorder, journey, start + 1, i - 1);

                // recursively construct the right subtree
                node->right = constructBST(preorder, journey, i, end);
