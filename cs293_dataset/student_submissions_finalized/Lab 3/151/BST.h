...
public:
	...
	// helper function in order to count jouneys in price bound
	// takes a ref of a vector, and pushed elements into it,
	// by using Inorder traversal, which performs treesort
	void getSorted(vector<int>& v, TreeNode* x);
	...
	// function that helps customStore function
	// helps to make a text file that stores the tree as
	//
	//
	// root->JourneyCode
	// root->price
	...
	//
	void storeHelper(string filename, TreeNode* x, int height = -1);
	...
	// helps to load only the left subtree
	// from a file that is specified
	void loadLeft(string filename);

	// helps to load the whole tree
	// from a file that is specified
	void loadFull(string filename);
