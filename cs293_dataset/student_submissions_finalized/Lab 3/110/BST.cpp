...
TreeNode* insertelement(TreeNode* node,int key,int JourneyCode){     // defining new function for using recursion
    if(node == NULL){
        ...
        node->left = insertelement(node->right,key,JourneyCode);
    }
    return node;
}
...
TreeNode* Search(TreeNode* node,int JourneyCode,int price){
    ...
    else Search(node->left,JourneyCode,price);
    return NULL;
}

bool BST::find(int JourneyCode, int price) { 
    TreeNode* t;
    t = Search(this->root,JourneyCode,price);
...
bool BST::remove(int JourneyCode, int price) { 
    ...
    this->root = deletion(this->root,price);
    return true;
}
