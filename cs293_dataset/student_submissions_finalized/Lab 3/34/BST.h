...
public:
        bool tree_insert(int Jrnycode, int jrnyprice)
        {
                if (this->price == 0 && this->JourneyCode == 0)
                {
                        this->price = jrnyprice;
                        this->JourneyCode = Jrnycode;
                        return true;
                }
                else if (this->price < price)
                {
                        ...
                        {
                                this->right = new TreeNode(Jrnycode, jrnyprice, nullptr, nullptr, this);
                                return true;
                        }
                        else{
                                this->right->tree_insert(Jrnycode, jrnyprice);
                        }
                }
                else{
                        if (this->left == nullptr)
                        {
                                this->left = new TreeNode(Jrnycode, jrnyprice, nullptr, nullptr, this);
                                return true;
                        }
                        else{
                                this->left->tree_insert(Jrnycode, jrnyprice);
                        }
                }
                return false;
        }

        struct TreeNode* tree_find(int Jrnycode, int Jrnyprice){
                if(this->JourneyCode == 0 && this->price)return nullptr;
                if(this->JourneyCode == Jrnycode && this->price == Jrnyprice)return this;
                else if(this->price < Jrnyprice){
                        this->right->tree_find(Jrnycode, Jrnyprice);
                }
                else{
                        this->left->tree_find(Jrnycode, Jrnyprice);
                }
        }
        struct TreeNode* smallest_node(){
                if(this->left == nullptr)return this;
                else return this->left->smallest_node();
        }
        void preorder(){
                if(this == nullptr)return;
                cout << this->JourneyCode << endl;
                this->right->preorder();
                this->left->preorder();
        }
        void postorder(){
