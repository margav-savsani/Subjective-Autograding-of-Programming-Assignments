...
void BST::traverse_dummy(int typeOfTraversal,TreeNode* root) {
    if(typeOfTraversal==0){
     if (root != NULL) {
      cout<<root->JourneyCode<<" ";
      traverse_dummy(0,root->left);
      traverse_dummy(0,root->right);
   }
    }
    if(typeOfTraversal==1){
     if (root != NULL) {
      traverse_dummy(1,root->left);
      traverse_dummy(1,root->right);
      cout<<root->JourneyCode<<" ";
   }
    }
    if(typeOfTraversal==2){
     if (root != NULL) {
      traverse_dummy(2,root->left);
      cout<<root->JourneyCode<<" ";
      traverse_dummy(2,root->right);
   }
    }
    return;
 }
