...
TreeNode* BST::prev(TreeNode *current){
    TreeNode *result_pre=NULL;
    TreeNode *result = current->left;
    while (result != NULL){
        result_pre = result;
        result = result->right;
    }
    //cout << "returning the prev" << result->JourneyCode << endl;
    return result_pre;
}

TreeNode *BST::give(TreeNode *root, int JourneyCode, int price){
    if (root == NULL){
        return NULL;
    }
    if (root->price == price){
        if (root->JourneyCode == JourneyCode){
            //cout << "returning the root 1" << endl;
            return root;
        }
        else{
            if (root->left == NULL){
