...
public:
        ...
        bool insert(int jc, int pc);

        // Return true, if the ticket with given attributes is found, false otherwise
        bool find(int JourneyCode, int price);
        struct TreeNode *find1(struct TreeNode* base);
        struct TreeNode *search(struct TreeNode* base,int pc,int jc);
        struct TreeNode *minimum(struct TreeNode* base);
        struct TreeNode *delete1(struct TreeNode*base,int jc, int pc);
        int inorderc(struct TreeNode*base,int count,int upperbound,int lowerbound);
        void preorder(struct TreeNode*base);
        void postorder(struct TreeNode*base);
