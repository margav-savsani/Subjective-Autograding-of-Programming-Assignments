...
bool BST::remove(int JourneyCode, int price, TreeNode *root){
    ...
    if (root->JourneyCode == JourneyCode && root->price == price){
        if(root->left == NULL){
            //Now we can simply remove this root
            if(root->parent==NULL){
                BST::root = root->right;
            }
            ...
            if(root->left->right==NULL){
                //remove root and add its right to root->left->right
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    int ans = 0;
    ans = countJourneysInPriceBound(lowerPriceBound, upperPriceBound , root , ans);
    return ans;
}

int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound , TreeNode *root , int ans){
    if(root==NULL) return ans;
    if(root->price>= lowerPriceBound && root->price<=upperPriceBound) ans ++;
    if(root->price >= lowerPriceBound) ans = countJourneysInPriceBound(lowerPriceBound, upperPriceBound , root->left , ans);
    if(root->price <= upperPriceBound) ans = countJourneysInPriceBound(lowerPriceBound, upperPriceBound , root->right , ans);
    return ans; 
}
