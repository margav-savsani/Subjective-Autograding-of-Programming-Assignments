...
public:
  ...
  int getheight(TreeNode *node){                    //introduced this function so that i dont need to update the height after every insertion and call this whenever required
    int left=0,right=0;
    TreeNode *x;
    x=node;
    if(x->left!=NULL){
      left=1+getheight(x->left);
    }
    if(x->right!=NULL){
      right=1+getheight(x->right);
    }
  ...
  void printInfo()
  {
    height=getheight(this);                             //update all the heights before calling the print function
  ...
  bool operator = (TreeNode const &node)                      //This is also created in order to implement the BSTEqualTo and AVLEqualTo functions.
  {
    if (JourneyCode == node.JourneyCode) return true;
    else return false;
  }
  ...
  int getjourneycode(){     //Created since we cant directly access the JourneyCode
    return JourneyCode;
  }

  int getprice(){            //Created since we cant directly access the price   
    return price;
  }
