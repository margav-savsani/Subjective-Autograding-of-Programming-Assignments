...
public:
  ...
  // Makes current node the left chil d of parentNode if
  ...
  bool updateParentChild(TreeNode *parentNode, bool isLeftChild)
  {
   ...
   void updateheight() {  //function to update the height of tree node : 

     if (left == nullptr && right == nullptr )   height  = 0 ;  // Case 1 : both childs are nullptr , then height is zero . 
     else if (left == nullptr)  height = right->height + 1 ; // Case 2 : left child is nullptr , then height is height of right + 1  . 
     else if (right == nullptr)  height = left->height + 1 ; // Case 3 : right child is nullptr , then height is height of left + 1  . 
     else  height = 1 + max(left->height,right->height); // Case 4 : both  child's are not null , then height is maximum of height of both childs + 1  . 
   }

   int balance() { // returns the balance of the tree . 
     int balance ; 
     if (left == nullptr && right == nullptr )   balance  = 0 ;  // if both childs are nullptr then balanced 
     else if (left == nullptr)  balance =  -right->height -1  ; // left is nullptr , then balance is height of rigth - 1
     else if (right == nullptr)  balance = left->height + 1 ; // left is nullptr , then balance is height of rigth - 1
     else  balance = left->height - right->height ; // both are not null , then balance is height of left - right child 
     return balance ;
   }
};
