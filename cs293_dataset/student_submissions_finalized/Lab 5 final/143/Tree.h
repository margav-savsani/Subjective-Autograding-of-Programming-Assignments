...
public:
  ...
  TreeNode() : price(0), JourneyCode(0), left(nullptr), right(nullptr) {height=0;} //constructor
  TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent) {} //constructor
  ...
  int journeycode(){
    return JourneyCode; //will return journeycode
  }
  int Price(){
    return price; //will return price
  }
  void setjc(int jc){
    JourneyCode = jc; //will change the journeycode
  }
  void setp(int p){
    price = p; //will change the price
  }
};
