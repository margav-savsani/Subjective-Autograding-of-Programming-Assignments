...
private:
  ...
  // Takes as input a child node and a parent node, and assigns them to each other
  // Has parameter isleftChild to determine which child it is
...
public:
  ...
  bool equal(TreeNode* one, TreeNode* two){
    return (!AVLLessThan(one, two) && !AVLLessThan(two, one));
  }
  // Destructor
  ~AVLTree() {
  ...
  TreeNode* getNode(int jrnyCode);
  TreeNode* twochild_successor(TreeNode* node);
