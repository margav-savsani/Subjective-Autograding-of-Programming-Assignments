...
private:
  ...
  void updateHeight(TreeNode *node){
    ...
    if(node->left == nullptr && node->right != nullptr) node->height = node->right->height+1;
    else if(node->left != nullptr && node->right == nullptr) node->height = node->left->height+1;
    else if(node->left == nullptr && node->right == nullptr) node->height = 0;
    else node->height = 1+max(node->left->height, node->right->height);
    if(node->height != temp && node->parent != nullptr) updateHeight(node->parent);
    else return;
  }

  void finalHeight(TreeNode *node){
    if(node->right != nullptr && node->left != nullptr) node->height = max(node->right->height, node->left->height)+1;
    else if(node->right == nullptr && node->left != nullptr) node->height = node->left->height+1;
    else if(node->right != nullptr && node->left == nullptr) node->height = node->right->height+1;
    else node->height = 0;
  }
