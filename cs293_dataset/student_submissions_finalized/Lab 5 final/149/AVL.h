...
public:
  ...
  bool addNode(TreeNode *t, int jrnycode, int price);
  int find(int jrnyCode);
  TreeNode *findNode(TreeNode *t, int code);
  void balance(TreeNode *t, TreeNode *y);

  /// @brief getheight difference of two nodes
  /// @param t node whose height is asked gor
  /// @return imbalance in height
  int getBal(TreeNode *t)
  {
    if (t == nullptr)
      return 0;
