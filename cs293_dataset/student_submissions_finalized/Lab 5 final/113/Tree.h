...
public:
  ...
  //returns price of node
  int getPrice(){
    return price;
  }
  //return journey code of node
  int getJC(){
    return JourneyCode;
  }
  ...
  //to update the height of given node for AVL nodes based on different cases and returning if the height changed or not to
  //check whether change values above this node or not
  bool updateAVLHeight(){
  ...
  //to update the height of given node for BST nodes based on different cases and returning if the height changed or not to
  //check whether change values above this node or not
  bool updateBSTHeight(){
    int prev_h=height;
    if (left != NULL && right != NULL){
      height = max(left->height,right->height) + 1;
    }
