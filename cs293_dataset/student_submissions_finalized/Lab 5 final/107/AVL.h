...
public:
  ...
  void updateAVLsingleheight(TreeNode* node);

  // updates height of nodes up the tree starting at parentOfCurrNode
  TreeNode* updateAVLheight(TreeNode* parentOfCurrNode);

  // rotates at passed node according to what situation demands
  void rotate(TreeNode* z);

  // Returns false, if given JourneyCode is already present in the AVL
	// Inserts the element and returns True otherwise
  bool insert(int jrnyCode, int price);

  // Return true, if the ticket with given jrnyCode is found, false otherwise
