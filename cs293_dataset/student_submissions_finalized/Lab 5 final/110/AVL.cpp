...
// tree.
TreeNode* AVLTree:: insertelement(TreeNode* root,int JourneyCode,int price,TreeNode* element){     // defining new function for using recursion
    ...
    else if(AVLLessThan(root,element)){
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  while (k!=NULL)
  {
    if(k->left == NULL && k->right->height>=1) break;
    if(k->right == NULL && k->left->height>=1) break;
    if(k->left!=NULL && k->right!=NULL){
      if(k->right->height-k->left->height>1 || k->left->height-k->right->height>1){
        break;
      }
    }
    if(k->parent!=NULL){  
  ...
  if(k!=NULL){  
    ...
    if(y!=NULL && z!= NULL){
      ...
      if(xleft && yleft){
        ...
        z->height-=2;
        TreeNode* dummy = y->parent;
        while(dummy != NULL){
          if (dummy->left != NULL && dummy->right != NULL){
            dummy->height = max(dummy->left->height, dummy->right->height) + 1;
          } else if (dummy->left == NULL && dummy->right != NULL){
            dummy->height = dummy->right->height + 1;
          } else {
            dummy->height = dummy->left->height + 1;
          }
...
int AVLTree::find(int jrnyCode){
  ...
  tr = root;
  while(tr!=NULL){
    if(!(AVLLessThan(tr,search)||AVLLessThan(search,tr))) return 1;
    else if(AVLLessThan(tr,search)) tr=tr->right;
    else tr = tr->left;
  }
  return -1;
}
