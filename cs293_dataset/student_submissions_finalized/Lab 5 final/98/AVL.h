...
public:
  ...
  // Implemented a helper function to use recursion to find the journies
  // Takes two arguments. Searches for comp starting at ptr as the root
  bool findjourney(TreeNode* comp,TreeNode* ptr);
  
  // Fucntion to change the root value to ptr
  // Used to perform rotations in which root node is being changed
  void changeroot(TreeNode* ptr)
  {
    root=ptr;
  }
};
