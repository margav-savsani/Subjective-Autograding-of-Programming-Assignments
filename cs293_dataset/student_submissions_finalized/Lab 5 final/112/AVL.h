...
private:
  ...
  void updateAVLParentChild(TreeNode *childNode, TreeNode *parentNode, bool isLeftChild)
  {
    ...
    {
      childNode->updateParent(parentNode, isLeftChild);
...
public:
  ...
  bool myInsert(int jrnyCode, TreeNode *insertInMe); // insert in subtree
  bool insert(int jrnyCode);
  int find(int jrnyCode);
  void adjust(TreeNode *addedNode); // rotate and modify heights after insertion
