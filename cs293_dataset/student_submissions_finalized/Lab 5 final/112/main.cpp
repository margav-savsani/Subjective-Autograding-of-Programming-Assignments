...
int main(int argc, char **argv)
{
  ...
  while (cin >> cmd)
  {
    if (cmd == "ADD")
    {
      ...
      if (((choice == 0) ? treeBST->insert(jCode) : treeAVL->insert(jCode)))
      {
    ...
    else if (cmd == "FIND")
    {
      ...
      bool found = ((choice == 0) ? treeBST->find(jCode) : treeAVL->find(jCode));
      if (found == true)
      {
        cout << "FOUND" << endl;
      }
    ...
    else if (cmd == "COMPCOST")
    {
      if (choice == 0)
      {
        cout << "Comparision cost is " << treeBST->getComparisonCost() << endl;
      }
      else
      {
        cout << "Comparision cost is " << treeAVL->getComparisonCost() << endl;
      }
    }
    else if (cmd == "SWAPCOST")
    {
      if (choice == 0)
      {
        cout << "Swap cost is " << treeBST->getParentChildUpdateCost() << endl;
      }
      else
      {
        cout << "Swap cost is " << treeAVL->getParentChildUpdateCost() << endl;
      }
    }
    else if (cmd == "EXIT")
    {
