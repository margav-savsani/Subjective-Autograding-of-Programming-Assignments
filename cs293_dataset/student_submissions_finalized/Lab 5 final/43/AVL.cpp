...
{ TreeNode *r;
  ...
  if(find(jrnyCode) == 0){
    ...
    while(gp != nullptr){
    if((gp->left == nullptr && gp->right->height ==2)||(gp->right == nullptr && gp->left->height == 2) ||
       !(gp->right->height - gp->left->height == 1 || gp->right->height - gp->left->height == -1 || gp->right->height - gp->left->height == 0)){
        rotate(j,p,gp);
        break;
    }
    else{
       if(gp->left == nullptr){
         gp->height = gp->right->height + 1;
       }
       else if(gp->right == nullptr){
         gp->height = gp->left->height + 1;
       }
       else if(gp->left != nullptr && gp->right != nullptr){
...
{  TreeNode *r = root;
   TreeNode *j;
   j = new TreeNode(jrnyCode,0);
   while(r!= nullptr && ( AVLLessThan(r,j) || AVLLessThan(j,r))){
        if(AVLLessThan(r,j)){
          r = r->right;
        }
        else{
