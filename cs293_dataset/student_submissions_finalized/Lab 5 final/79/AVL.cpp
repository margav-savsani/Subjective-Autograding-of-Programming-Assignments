...
void AVLTree::rotCall(TreeNode* node){
  TreeNode *Parent=node->parent;
  if(Parent==NULL)return;
  TreeNode* grandparent=Parent->parent;
  if(grandparent==NULL)return;
  bool left1=true,left2=true;
  if(Parent->right==node)left2=false;
  if(grandparent->right==Parent)left1=false;

  if(left1 == NULL){
    if(grandparent->left==NULL){
      if(left2){
        rotateRightLeft(node,Parent,grandparent);
        return;
      }
      else{
        rotateRightRight(node,Parent,grandparent);
        return;
      }
    }
    else if(abs((grandparent->left->height) - (grandparent->right->height))>1){
      if(left2){
        rotateRightLeft(node,Parent,grandparent);
        return;
      }
    ...
    if(grandparent->right == NULL){
      if(left2){
        rotateLeftLeft(node,Parent,grandparent);
        return;
      }
      else{
        rotateLeftRight(node,Parent,grandparent);
        return;
      }
    }
    else if(abs(grandparent->left->height-grandparent->right->height)>1){
...
bool AVLTree::rotateLeftLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  if(T3!=NULL)T3->parent=grandParentNode;
...
bool AVLTree::rotateLeftRight(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  if(greatGrandParentNode==NULL){
    ...
    currNode->parent=greatGrandParentNode;
  }
  ...
  if(T2!=NULL)T2->parent=parentNode;
  // We use "true" as the third argument of the following call to indicate
  // that T3 should become the left child of grandParentNode
  updateAVLParentChild(T3, grandParentNode, true);
  if(T3!=NULL)T3->parent=grandParentNode;
...
bool AVLTree::rotateRightLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  if(T2!=NULL)T2->parent=grandParentNode;
  // We use "true" as the third argument of the following call to indicate
  // that T3 should become the left child of grandParentNode
  updateAVLParentChild(T3,parentNode, true);
  if(T3!=NULL)T3->parent=parentNode;
  parentNode->height--;
  updateheight(grandParentNode);
  return true;
}
