...
bool AVLTree::rotateRightLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  // Let T3 be the right child of currNode (i.e. of x)
  TreeNode *T3 = currNode->right;
  if (greatGrandParentNode == nullptr) //handle the case when the greatGrandParent is null
  {
    root = currNode;
    root->parent = nullptr;
  }
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  if (AVLLessThan(parentOfCurrNode, newNode))
  {
  ...
  {
    int heightLeft = parentOfCurrNode->left ? parentOfCurrNode->left->height : -1;
    int heightRight = parentOfCurrNode->right ? parentOfCurrNode->right->height : -1;
    if (abs(heightLeft - heightRight) > 1 && !identifiedGP)
    {
      identifiedGP = 1;
      GP = parentOfCurrNode;
    }
    parentOfCurrNode->height = 1 + max(heightLeft, heightRight);
    parentOfCurrNode = parentOfCurrNode->parent;
  }

  if (GP != nullptr)
  {
    TreeNode *P = AVLLessThan(newNode, GP) ? GP->left : GP->right;
    TreeNode *C = AVLLessThan(newNode, P) ? P->left : P->right;
    balance(GP,P,C);
  }
...
int AVLTree::find(int jrnyCode)
{
  ...
  // see fit.
  TreeNode *node = root;
  TreeNode *dummy = new TreeNode(jrnyCode, 69);
  while (node != NULL)
  {
...
bool AVLTree::remove(int jrnyCode)
{
  ...
  TreeNode *parent = remove(delNode, jrnyCode);

  bool identifiedGP = false;
  TreeNode *GP;
  while (parent != nullptr)
  {
    int heightLeft = parent->left ? parent->left->height : -1;
    int heightRight = parent->right ? parent->right->height : -1;
    if (abs(heightLeft - heightRight) > 1 && !identifiedGP)
    {
      identifiedGP = 1;
      GP = parent;
      if (GP != nullptr && (GP->left!=nullptr || GP->right != nullptr))
      { 
        int GPL = GP->left?GP->left->height:-1;
        int GPR =  GP->right?GP->right->height:-1;
        TreeNode *P = GPL > GPR? GP->left : GP->right;
        // if(P == nullptr) break;
        int PL = P->left?P->left->height:-1;
        int PR = P->right?P->right->height:-1;
       
        TreeNode *C = PL > PR? P->left : P->right;
