...
// deleting nodes
void AVLTree::updateHeight(TreeNode* node){   //updates the height of the given node
  if(node->left==nullptr && node->right==nullptr){  //if the node is a leaf
    node->height = 1;
  }
  else if(node->left==nullptr && node->right!=nullptr){ //if the node has only right child
    node->height = node->right->height + 1;
  }
  else if(node->left!=nullptr && node->right==nullptr){ //if the node has only left child
...
void AVLTree::balance(TreeNode* Parent, TreeNode* curr){  //balances the tree by calling upon appropriate rotation functions
  TreeNode* GrandParent = Parent->parent;
  if(curr == Parent->left && Parent == GrandParent->left){  //left left case
    rotateLeftLeft(curr, Parent, GrandParent);
    GrandParent->height = GrandParent->height - 1;
  }
  else if(curr == Parent->right && Parent == GrandParent->left){  //right left case
    rotateLeftRight(curr, Parent, GrandParent);
    curr->height = curr->height + 1;
    Parent->height = Parent->height - 1;
    GrandParent->height = GrandParent->height - 1;
  }
  else if(curr == Parent->right && Parent == GrandParent->right){ //right right case
    rotateRightRight(curr, Parent, GrandParent);
    GrandParent->height = GrandParent->height - 1;
  }
  else if(curr == Parent->left && Parent == GrandParent->right){  //left right case
...
void AVLTree::updateHeightandBalance(TreeNode* node){ //update the height starting from the given node and balance upon encountering an imbalance
  TreeNode* newCurr = NULL;
  TreeNode* newnode = node;
  TreeNode* sibling = NULL;
  TreeNode* curr = NULL;
  int hdiff;
  while(newnode!=nullptr){  //updates height and balances unless newnode becomes parent of the root
    newCurr = newnode;
    updateHeight(newCurr);  //update height
    if(newCurr->height >= 2){   //there can be no imbalance for height less than 2
      if(newCurr->parent!=nullptr){
        if(newCurr == newCurr->parent->left){
          sibling = newCurr->parent->right;
        }
        else{
          sibling = newCurr->parent->left;
        }
        if(sibling!=nullptr){
          hdiff = abs(newCurr->height - sibling->height);
        }
        else{
          hdiff = newCurr->height;
        }
        if(hdiff>=2){   //call for balance if imbalance is detected
          balance(newCurr, curr);
        }
      }
    }
    curr = newnode; //useful for storing the previous node for balancing
...
bool AVLTree::rotateLeftLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  else{   //needed if grandParentNode is the root
    root=parentNode;
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  else if(AVLLessThan(node2b, tempp)){
    tempp->left = node2b;
  }
  else{
    tempp->right = node2b;
  }
  updateHeightandBalance(node2b);   //update height and balance after insertion
  return true;
}
...
int AVLTree::find(int jrnyCode)
{
  ...
  while(tempn!=nullptr){
    if(AVLLessThan(disposable, tempn)){
      tempn = tempn->left;
    }
    else if(AVLLessThan(tempn, disposable)){
      tempn = tempn->right;
    }
    else break;
  }
...
bool AVLTree::remove(int jrnyCode)
{
  ...
  if(x!=nullptr){
    ...
    updateHeightandBalance(node->parent);
  }
  return true;
}
