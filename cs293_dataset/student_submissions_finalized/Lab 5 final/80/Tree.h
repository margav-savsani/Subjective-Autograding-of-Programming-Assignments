...
public:
  ...
  int check_imbalance(){ //function to check for imbalance in the given node. Taking into account cases with 0-2 children.

    if(left == nullptr && right==nullptr){//no imbalance case
      return 0;
    }
    if(left==nullptr && right->height > 0){
      return 2; //corresponds to rotate on right side
    }
    if(right==nullptr && left->height > 0){
      return 1; //corresponds to rotate on left side
    }
    if(left==nullptr || right == nullptr){//no imbalance case
      return 0;
    }
    ...
    return 0;//corresponds to no imbalance case
