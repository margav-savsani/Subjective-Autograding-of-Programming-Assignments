...
private:
  ...
  // member functions to do rotation during insertion
  ...
  void updateAVLParentChild(TreeNode *childNode, TreeNode *parentNode, bool isLeftChild)
  {
    ...
    else if ((childNode != nullptr) && (parentNode == nullptr))
    {
      ...
      //If the parentNode provided is nullptr that means root is being modified
      // No updating of totalParentChildUpdateCost needed here
    }
    else
    {
...
public:
  ...
  TreeNode *updateheight(TreeNode *z);

  //When provided the current node, checks which rotation to perform on it(like leftleft or leftright etc) and does it
  void dorotation(TreeNode *x);

  // Optional: Implement "remove" only for bonus marks(not done)
