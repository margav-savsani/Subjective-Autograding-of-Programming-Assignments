...
public:
  ...
  void updateHt(TreeNode *newRoot){
    if(newRoot == NULL) return;
    if(newRoot->left == nullptr && newRoot->right != nullptr){
      newRoot->height = newRoot->right->getHt() +1;
    }
    else if(newRoot->left != nullptr && newRoot->right == nullptr){
      newRoot->height = newRoot->left->getHt() +1;
    }
    else if(newRoot->left != nullptr && newRoot->right != nullptr){
      newRoot->height = max(newRoot->left->getHt(), newRoot->right->getHt()) +1;
    }
    else
      newRoot->height = 0;
  }
  void updateHtTree(TreeNode *node){
    while(node != nullptr){
      updateHt(node);
      node = node->parent;
    }
  ...
  int getBalance(TreeNode *node){
    if (node == NULL)
      ...
      return node->left->getHt() - node->right->getHt();
  }
