...
private:
  ...
  TreeNode* findAndReturn(int JourneyCode);
  ...
  void updateHeightsAndRebalance(TreeNode* node, bool fix_one=false);
  TreeNode* updateHeights(TreeNode* node, bool report_imbalance=true);
  bool updateNodeHeight(TreeNode* node);
  TreeNode* getTallerChild(TreeNode* node);
