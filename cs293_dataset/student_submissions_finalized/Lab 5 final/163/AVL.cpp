...
bool AVLTree::rotateLeftLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
    ...
    if (root == grandParentNode)
        root = parentNode; // root of true needs to be updated if it was grandParentNode
...
bool AVLTree::rotateLeftRight(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
    ...
    if (root == grandParentNode)
        root = currNode; // root of true needs to be updated if it was grandParentNode
...
bool AVLTree::insert(int jrnyCode, int price)
{
    ...
    {
        if (AVLLessThan(temp, test)) // in this case, we need to insert in left of test
        {
            parent = test;
            test = test->left;
        }
    ...
    updateAVLParentChild(temp, parent, AVLLessThan(temp, parent)); // temp is insert as child of parent, at left or right depeneding on which one is less

    TreeNode *currNode = temp, *parentNode = parent, *grandParentNode = parent->parent;
    int h = 1;                 // height
    while (parentNode != root) // We need to update height of nodes in the path from the last parent to root only
    {
        if (h > parentNode->height) // If the height of last node inserted is more than current height, height is updated
            parentNode->height = h++;
        if (grandParentNode->heightDifference() > 1)
        {
    ...
    if (h > parentNode->height) // height of root is updated
        parentNode->height = h;
...
int AVLTree::find(int jrnyCode)
{
    ...
    {
        if (AVLLessThan(temp, test))
        {
            test = test->left; // If temp < test, we check in left of test
        }
        else if (!AVLLessThan(test, temp))
        {
