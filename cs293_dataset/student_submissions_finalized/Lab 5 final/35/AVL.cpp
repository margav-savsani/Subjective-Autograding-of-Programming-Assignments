...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  TreeNode * New = new TreeNode(jrnyCode,price);
    ...
    else if(AVLLessThan(New,y)){
        updateAVLParentChild(New,y,true);
    }
    else if(AVLLessThan(y,New)){
        updateAVLParentChild(New,y,false);
    }
    if(New->parent != nullptr){
    if(New->parent->parent != nullptr){
    TreeNode * curr = New;
    TreeNode * par = New->parent;
    TreeNode * grandpar = New->parent->parent;
    while(grandpar != nullptr){
      TreeNode * L = par;
      ...
      int imbalance;
      if(grandpar->left == nullptr){imbalance = grandpar->right->height + 1;}
      else if(grandpar->right == nullptr){imbalance = grandpar->left->height + 1;}
