...
public:
  ...
  // Below 6 functions are extra functions that i have added for some functionaliy
  
  int hd(TreeNode *x);                                        //function for calculating difference between left and right subtrees at given TreeNode.
  int higher(TreeNode *x);                                    //function for knowing which subtree have greater height at given TreeNode
  int abs(int x,int y);                                       // function for calculating absolute difference between integers
  void updateheight(TreeNode *x);                             //function for updating heights after inserting a element
  TreeNode *parent(TreeNode *root,TreeNode node);             // function for finding parentnode for element being added 
  TreeNode* findelement(TreeNode* x,TreeNode node);           // function to return pointer pointing to that node if present otherwise returns nullptr
