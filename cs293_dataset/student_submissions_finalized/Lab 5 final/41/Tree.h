...
public:
    ...
    // this function updates the height of the tree
    // hgt_l stores the height of left subtree whereas
    // hgt_r stores the height of the right subtree
    // this function stores the height of every node in the private variable 'height'
    void update_the_height(){
        int hgt_l = -1, hgt_r = -1;
        if(left!=nullptr) hgt_l = left->height;
        if(right!=nullptr) hgt_r = right->height;
        height = max(hgt_l,hgt_r) + 1;
    }
    
    // this function returns the imbalace in the tree
    // hgt_l stores the height of left subtree whereas
    // hgt_r stores the height of the right subtree
    int get_imbalance(){
        int hgt_l = -1, hgt_r = -1;
        if (left != nullptr) hgt_l = left->height;
        if (right != nullptr) hgt_r = right->height;
        return abs(hgt_l - hgt_r);
    }
    
    // this function returns the price associated with the node,
    // which is a private variable in the class
    int get_price(){
        return price;
    }
    
    // this function returns the JourneyCode associated with the node,
    // which is a private variable in the class
    int get_journey(){
        return JourneyCode;
    }
