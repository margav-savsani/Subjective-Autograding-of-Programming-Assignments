...
public:
  ...
  // copy operator defined to copy only the journeycode and price.
  void operator = (TreeNode const &node)
  {
    JourneyCode = node.JourneyCode;
    price = node.price; // only the price and journeycode.
  }
  int getPrice(){ // returns the price of the tree node.
    return price;
  }
  ...
  bool updateParentChild(TreeNode *parentNode, bool isLeftChild)
  {
    if (parentNode != nullptr) {
      parent = parentNode; // one way matched
      if (isLeftChild) {
	      parentNode->left = this;// second way done
      }
      else {
