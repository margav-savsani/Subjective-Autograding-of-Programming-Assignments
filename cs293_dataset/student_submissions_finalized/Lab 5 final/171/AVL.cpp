...
int AVLTree:: getbalance(TreeNode *z){
  return (z->right->height)-(z->left->height);
}
...
{ 
    ...
    if(find(jrnyCode)==price)
     ...
     TreeNode *b=a;         //child
     TreeNode *p=a->parent;   //parent
     TreeNode *q=p->parent;  //grand parent
     if(q==NULL) {return true;}
     while(q!=NULL && q->right!=NULL && q->left!=NULL){
      
      int bal=getbalance(q);
     
      if(bal<-1 || bal>1){
         break;               //if balancefactor in [-1,1] breaks the while
        }
        else {
          b=p;
          p=q;
    ...
    if(q->left==p && p->left==b) return rotateLeftLeft(b,p,q);   //rotate LL
    else if(q->right==p && p->right==b) { return rotateRightRight(b,p,q);}  //rotate RR
    else if(q->left==p && y->right==b) return rotateLeftRight(b,p,q);      //rotate LR
    else return rotateRightLeft(b,p,q);        //rotate RL
