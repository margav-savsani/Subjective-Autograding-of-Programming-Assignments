...
void AVLTree::update(int jrnyCode){
  ...
  while(currNode!=nullptr){
    ...
    else if(AVLLessThan(currNode,nemo)){
      int m=currNode->highestlength();
      if(m > currNode->height) currNode->height=m;
...
void AVLTree::update1(int jrnyCode){
  ...
  while(currNode!=nullptr){
    if(AVLLessThan(nemo,currNode)){
      currNode->height=currNode->highestlength();
      currNode=currNode->left;
    }
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  while (currNode != nullptr) {
    if (AVLLessThan(currNode,childNode)) {
      parentOfCurrNode = currNode;
      currNode = currNode->right;
    }
    else if (AVLLessThan(childNode,currNode)) {
      parentOfCurrNode = currNode;
      currNode = currNode->left;
    }
    else return false;
  ...
  if(parentOfCurrNode!=nullptr){
    bool isleft=AVLLessThan(childNode,parentOfCurrNode);
    updateAVLParentChild(childNode,parentOfCurrNode,isleft);
    update(jrnyCode);
  }
  ...
  class TreeNode* child=childNode,*parent=child->parent,*grandparent=parent->parent;
  char c1,c2;
  while(grandparent!=nullptr){

    int x,y;
    if(grandparent->left==nullptr){x=-1;}
    else x=grandparent->left->height;

    if(grandparent->right==nullptr){y=-1;}
    else y=grandparent->right->height;
    ...
    child=parent;parent=grandparent;grandparent=grandparent->parent;
  }
...
int AVLTree::find(int jrnyCode)
{
  ...
  while (currNode != nullptr) {
    if (AVLLessThan(currNode,nemo)) {
      parentOfCurrNode = currNode;
      currNode = currNode->right;
    }
    else if (AVLLessThan(nemo,currNode)) {
      parentOfCurrNode = currNode;
      currNode = currNode->left;
    }
