...
bool AVLTree::rotateLeftLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  updateHeightAlongPath(parentNode);
...
bool AVLTree::rotateLeftRight(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  updateHeightAlongPath(currNode);
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  else if(AVLLessThan(node,y)){
      updateAVLParentChild(node,y,true);
  }
  else updateAVLParentChild(node,y,false);

  //update heights of nodes from inserted node to root node
  updateHeightAlongPath(node);
  

  //find the first imbalance in the tree, along the path from inserted node to root node
  TreeNode* grandpa=findgrandpa(node);

  //using the grandpa node, find parent and child nodes
  if(grandpa!=NULL){

    TreeNode* parent=NULL;
    TreeNode* child=NULL;
    bool l;
    if(AVLLessThan(node,grandpa)){
      parent=grandpa->left;
      l=true;
    }
    else {
    ...
    if(AVLLessThan(node,parent)){
      child=parent->left;
      if(l)
        rotateLeftLeft(child,parent,grandpa);
      else
        rotateRightLeft(child,parent,grandpa);
    }
    else {
      child=parent->right;
      if(l)
        rotateLeftRight(child,parent,grandpa);
      else{
        rotateRightRight(child,parent,grandpa);
      }
    }
  }
...
bool AVLTree::updateHeightAlongPath(TreeNode* node){
    TreeNode *y, *x;
    y=NULL;
    x=node;
    while(x!=NULL){
...
TreeNode* AVLTree::findgrandpa(TreeNode* node){
  TreeNode* y2=NULL;
  TreeNode* x2=node;
  ...
  while(x2!=NULL){
    y2=x2;
    x2=x2->parent;
    if(y2->left==NULL && y2->right==NULL){}
    else if(y2->left==NULL && y2->right!=NULL){
      if((y2->right)->height>=1){
        grandpa=y2;
        break;
      }
    }
    else if(y2->right==NULL && y2->left!=NULL){
      if((y2->left)->height>=1){
        grandpa=y2;
        break;
      }
    }
    else if((y2->right)->height>(y2->left)->height+1 || (y2->left)->height>(y2->right)->height+1){
        grandpa = y2;
        break;
    }
  }
