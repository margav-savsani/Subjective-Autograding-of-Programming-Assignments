...
// deleting nodes
int AVLTree::getHeight(TreeNode* x) {
...
bool AVLTree::heightCheck(TreeNode *x) {
  if(x->left==nullptr&&x->right==nullptr){
    return true;
  }
  else if(x->left!=nullptr&&x->right==nullptr){
    if(getHeight(x->left)>1) return false;
    else return true;
  }
  else if(x->left==nullptr&&x->right!=nullptr){
    if(getHeight(x->right)>1) return false;
    else return true;
  }
  else{
    if(abs(getHeight(x->left)-getHeight(x->right))>1) return false;
    else return true;
  }
...
// tree.
bool AVLTree::Insert(int jrnyCode, int price, TreeNode* x)
{
...
if(x==nullptr) {
  ...
  t->price=price;
  if(!(AVLLessThan(t,x))&&!(AVLLessThan(x,t))) {
    return false;
  }
            ...
            if(x->left==nullptr) {
                ...
                if(leftChild(y,z)&&leftChild(x,y)){
                  return rotateLeftLeft(x,y,z);
                }
                else if(leftChild(y,z)&&rightChild(x,y)) {
                  return rotateLeftRight(x,y,z);
                }
                else if(rightChild(y,z)&&leftChild(x,y)){
                  return rotateRightLeft(x,y,z);
                }
                else if(rightChild(y,z)&&rightChild(x,y)){
...
// code, and journey code is the key for insertion and finding in the AVL
// tree.
int AVLTree::Find( int jrnyCode,TreeNode *x) {
   if(x==nullptr) return -1;
    ...
    else if (AVLLessThan(x,t))
    {
    
      AVLTree rightAVLTree(x->right,1,1);
      
      return rightAVLTree.Find(jrnyCode,x->right);
      
    }
    else{
      
      AVLTree leftAVLTree(x->left,1,1);
      
      return leftAVLTree.Find(jrnyCode,x->left);
    }
}
