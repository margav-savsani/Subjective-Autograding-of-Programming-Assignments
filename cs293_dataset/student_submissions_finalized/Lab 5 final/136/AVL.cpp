...
// deleting nodes
bool AVLTree::checkBalance(TreeNode* p){
  if(p->left==nullptr){
    if(p->right==nullptr) return true;
    else return (p->right->height<1);
  }
  if(p->right==nullptr){
    return (p->left->height<1);
  }
  return ((p->left->height-p->right->height-1)*(p->left->height-p->right->height+1)<=0);
}
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  while(true)
  {
    ...
    // cout<<grandparent->getjrny()<<endl;
    // cout<<checkBalance(grandparent)<<endl;
  ...
  if(gparent->right==parent){
    if(parent->right==curr){
      rotateRightRight(curr,parent,gparent);
    }
    else{
      parent->height--;
      curr->height++;
      rotateRightLeft(curr,parent,gparent);
    }
  }
  else{
    if(parent->right==curr){
      parent->height--;
      curr->height++;
      rotateLeftRight(curr,parent,gparent);
    }
    else{
      rotateLeftLeft(curr,parent,gparent);
    }    
  }
  return true;
}
...
bool AVLTree::remove(int jrnyCode)
{
  ...
  while(true)
  {
    ...
    if(!checkBalance(par->parent)){
      ...
      gparent->height-=2;
      if(gparent->left->height==gparent->height-1){
        parent=gparent->left;
      }
      else{
        parent=gparent->right;
      }
      if(parent->left->height==parent->height-1){
        curr=parent->left;
      }
      else{
        curr=parent->right;
