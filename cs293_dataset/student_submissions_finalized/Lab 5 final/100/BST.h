...
private:
  ...
  //returns node with given JourneyCode if it is present in tree
  //else returns null 
  TreeNode* getNode(int JourneyCode){
    TreeNode *x,*temp;
    temp=new TreeNode(JourneyCode,0);
    x=root;
    while(x!=NULL){
...
//returns successor of given node. if node itself is null then returns null
