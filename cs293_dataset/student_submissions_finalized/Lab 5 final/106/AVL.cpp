...
bool AVLTree::rotateRightLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  // Now we have to assign childs of currNode
  // Let T2 be the left child of currNode
  TreeNode *T2 = currNode->left;
  
  // Let T3 be the right child of currNode
  TreeNode *T3 = currNode->right;

  // T2 becomes right child of grandParent Node
  updateAVLParentChild(T2, grandParentNode, false);

  // T3 becomes left child of parent Node
...
bool AVLTree::rotateRightRight(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  // Now we have to assign childs of currNode
  // right child of currNode remains same
  // Let T2 be the left child of currNode
  TreeNode *T2 = currNode->left;;
...
void AVLTree::balance(TreeNode* n) { // it recursively balances left child tree and right child tree
  if (n == NULL) return;
  balance(n->left);
  balance(n->right);
  updateH(n);                        // updates heights after balncing children
  int b = n->getBalance();
  if(b>1) {
    int c = n->left->getBalance();
    if(c>=1) rotateLeftLeft(n->left->left, n->left, n);
    if(c<=-1) rotateLeftRight(n->left->right, n->left, n);
  }
  if(b<-1) {
    int c = n->right->getBalance();
    if (c>=1) rotateRightLeft(n->right->left, n->right, n);
    if (c<=-1) rotateRightRight(n->right->right, n->right, n);
  }
  updateH(n);                        // updates heights after balncing completely
}
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  while (true) {
    if (currNode == NULL) {
      ...
      updateAVLParentChild(newNode,parent,flag);
      break;
    }
    if (AVLLessThan(newNode, currNode)) {
      flag = 1;                             // new node is left child of parent
      parent = currNode;
      currNode = currNode->left;
    }
    else if (AVLLessThan(currNode, newNode)) {
  ...
  balance(root);                            // balances tree after insertion
  return true;
  }
...
int AVLTree::find(int jrnyCode)
{
  ...
  while(true) {
    ...
    if (AVLLessThan(newNode, currNode)) currNode = currNode->left;
    if (AVLLessThan(currNode, newNode)) currNode = currNode->right;
