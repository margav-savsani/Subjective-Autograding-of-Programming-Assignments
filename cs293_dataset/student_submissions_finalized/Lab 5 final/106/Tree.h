...
public:
  ...
  int getjrnycode () { //to access journey code of a node
    return JourneyCode;
  }

  int getprice () { // to access price of a node
    return price;
  }

  int getBalance() {    // gives balance of heights of reight child and left child of a treenode
    if (this == NULL)
        return 0;
...
int updateH (TreeNode* node) { //to recursively update the height of a node by updating heights of children
  if (node == NULL) return -1;
  int rh = updateH(node->right);
  int lh = updateH(node->left);
