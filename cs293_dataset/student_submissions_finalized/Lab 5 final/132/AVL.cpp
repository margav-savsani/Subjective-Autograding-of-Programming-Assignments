...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  if(find(jrnyCode)!=-1) return false;
  	...
  	z->height=1;
  	y->left=z;
  	y->height=max(z, y->right->height)
  	}
  	else{
  	z->parent=y;
  	z->height=1;
  	y->right=z;
  	y->height=max(z->height, y->right->height)
  	}
  	updateheight(z);
  }
  
  TreeNode *currNode= getimbalance(z);
  ...
  if(grandparentNode->left==parentNode){
  	if(parentNode->left==root){rotateLeftLeft(currNode, parentNode, grandparentNode); grandparentNode->height-=2;}
  	...
  	if(parentNode->right==currNode){ rotateRightRight(currNode, parentNode, grandparentNode); grandparentNode->height-=2;}
  	else if(parentNode->right==currNode){
  		rotateRightLeft(currNode, parentNode, grandparentNode);
  		currNode->height++;//height changes after rotate
  		parentNode->height--;
  		grandparentNode->height-=2;
  	}
  }
...
int AVLTree::find(int jrnyCode)
{
  ...
  // see fit.
  TreeNode *x=root;
  TreeNode *y=new TreeNode(jrnyCode, 0);//creating a dummy treenode whose jrnycode we have to find in THIS tree
  while(x != nullptr){
  	if(x- isequal(jrnyCode)){
  	totalComparisonCost += perComparisonCost;//we didn't use BSTLessThan function so we will have to do this manually here.
  	return x->getprice();
  	}
