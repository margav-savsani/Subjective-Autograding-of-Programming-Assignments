...
private:
  ...
  bool isLeftChild(TreeNode* a, TreeNode* b);
  ...
  int imbalance(TreeNode* t){
  	if( t->right==nullptr && t->left==nullptr ) return 0;
  	else if( t->right==nullptr ) return (t->left->height)+1;
  	else if( t->left==nullptr ) return (t->right->height)+1;
