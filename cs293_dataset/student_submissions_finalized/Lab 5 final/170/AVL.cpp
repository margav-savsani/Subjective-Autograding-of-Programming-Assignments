...
bool AVLTree::isLeftChild(TreeNode* a, TreeNode* b){
...
bool AVLTree::rotateLeftLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  updateAVLParentChild(parentNode, greatGrandParentNode, isLeftChild(greatGrandParentNode, grandParentNode));
  ...
  grandParentNode->height=parentNode->height-1;
  while(parentNode->parent!=NULL){
      parentNode->parent->height=parentNode->parent->height-1;
...
bool AVLTree::rotateLeftRight(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  // child of greatGrandParentNode
  updateAVLParentChild(currNode, greatGrandParentNode, isLeftChild(greatGrandParentNode, grandParentNode));
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  // see fit.
	...
	while(x!=nullptr){
		...
		y = x;
		z = x->parent;
		if(AVLLessThan(x,n)) x = x->right;
		else x = x->left;
	}
	n->parent = y;
	if( y == nullptr){ root = n; return true; }
	else if(AVLLessThan(y,n)){
             y->right=n;
             TreeNode *p=y->right;
             if(y->left==NULL){
	...
	if( y == nullptr || z == nullptr ) return true;
	while(z!=nullptr){
		if(imbalance(z)>1 || imbalance(z)<-1){
		
			if(isLeftChild(z,y)){
				if(isLeftChild(y,x)) return rotateLeftLeft(x, y, z);
				else return rotateLeftRight(x, y, z);
			}	
			else{
				if(isLeftChild(y,x)) return rotateRightLeft(x, y, z);
				else return rotateRightRight(x, y, z);	
			}	
		}
		else { 
			x=y;
...
int AVLTree::find(int jrnyCode)
{
  ...
  // see fit.
	...
	while( x!=nullptr ){
		...
		else if(AVLLessThan(z,x)) x = x->left;
		else x = x->right;
	}
	return -1;
}
