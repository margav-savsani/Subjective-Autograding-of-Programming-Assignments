...
public:
  ...
  // Updates Height of current node using children nodes data
  // Returns true if height gets updated to a new value
  // Returns false if height remains unchanged after updated
  bool updateHeight(){
    int newHeight;

    // Both child present
    if(left != nullptr && right != nullptr){
      newHeight = max(left->height, right->height) + 1;
    }
    ...
    // If height remains unchanged return false
    if(newHeight == height){
      return false;
    }
    // Else update height and return true
    else{
      height = newHeight;
      return true;
    }
  ...
  // Used only for returning price in find function
  // And copying successor in remove function
  ...
  void setJourneyCode(int jc){
    JourneyCode = jc;
  }


  // Returns imbalance at current node
  // Positive means left has more height
  // Negative means right has more height
  int getImbalance(){
    int imbalance;
    // Both child present
    if(left != nullptr && right != nullptr){
      imbalance = left->height - right->height;
    }
    // Only left child present
    else if(left != nullptr){
      imbalance = left->height+1;
    }
    // Only right child present
    else if(right != nullptr){
      imbalance = -(right->height+1);
    }
    // No child present
    else{
      imbalance = 0;
  ...
  // Returns false if imbalance is neither -1, 0 or 1
  // Else returns true
  bool isBalanced(){
