...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  TreeNode *k= new TreeNode(jrnyCode,price);
  ...
  TreeNode *curr = root;
  while(true){
    if(AVLLessThan(k,curr)){
      if(curr->left!=nullptr){
        curr=curr->left;
      }
      else{
        //Making k the left child
        updateAVLParentChild(k,curr,true);
        curr=curr->left;
        break;
      }
    }
      ...
      if(curr->right!=nullptr){
        ...
        updateAVLParentChild(k,curr,false);
        curr=curr->right;
        break;
      }
    } 
...
int AVLTree::find(int jrnyCode)
{
  ...
  //price is -1  
  TreeNode *k= new TreeNode(jrnyCode,-1);
  while(true){
    ...
    else if(AVLLessThan(curr,k)){
      if(curr->right==nullptr){
        //if not found returning -01
        return -1;
      }
      else{
        curr=curr->right;
