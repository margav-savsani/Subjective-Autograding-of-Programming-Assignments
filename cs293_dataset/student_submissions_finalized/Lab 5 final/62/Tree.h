...
public:
	...
	TreeNode(int jrnyCode, int jrnyPrice, TreeNode* parptr)
	{
		JourneyCode = jrnyCode;
		price = jrnyPrice;
		left = nullptr;
	...
	// new operator overloading , helps w rem_node
	void operator= (TreeNode const &other)
	{
		this->JourneyCode = other.JourneyCode;
...
void printBinaryTree(TreeNode *root, const string& prefix, bool isLeft)
{
	...
	else { // changed slightly
		cout << prefix + (isLeft ? "|--" : "|__" ) + "o" << endl;
	}
}
