...
private:
	...
	int getHeight(TreeNode* toget){
		if (toget==nullptr) return -1;
		return toget->height;
	}
	//updates heights and implements rotation in the general case
	bool updateHeights(TreeNode* leaf);
...
public:
	...
	//helper functions to remove a node
	TreeNode* rem_node(TreeNode* node);
