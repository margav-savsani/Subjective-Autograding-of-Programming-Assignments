...
template <typename T> void DynamicQueue<T>::QInsert(T x) {
    ...
    else if(tail>=N){ //this is the case where the head index is not at 0 but the tail index reaches N, in this case the queue is not full but the empty elements are not at the end
