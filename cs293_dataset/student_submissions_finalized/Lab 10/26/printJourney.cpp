...
{
  int id1;
  id1=stnNameToIndex.get(srcStnName)->value;
  listOfObjects<StationConnectionInfo *> *overall;
  int num = 0;
  listOfObjects<StationConnectionInfo *> *prefinal;
  listOfObjects<StationConnectionInfo *> *intermediate = adjacency[id1].toStations;
  ...
  while (overall != nullptr)
  {
    ...
    {
      traincode = jc->object;
      if (traincode == traingoing->object->journeyCode || traincode == -1  || limit>=0)// matching the current code with the traincode stored in queue for conforming direct journey. If the station is adjacent to source, we dont need to match codes, hence -1
      {
        if(!(traincode == traingoing->object->journeyCode )){
          if (traincode!= -1){
            limit=limit-1;
            excedd=1;
          }
        ...
        if(traingoing->object->depTime-lasttrain->object->arrTime<=maxTransitTime){
        int adjstn;
      ...
      listOfObjects<TrainInfoPerStation *> *lasttrain=traingoing;
