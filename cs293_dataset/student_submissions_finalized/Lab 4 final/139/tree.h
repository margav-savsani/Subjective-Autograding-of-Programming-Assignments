...
struct TreeNode
{
  ...
  // bool operator<(const TreeNode &leftArgument, const TreeNode &rightArgument)
  // {
...
class BST
{
  ...
  TreeNode *root;
  int shortestPathLength, longestPathLength; // length of shortest path,longestPathLength
  vector<int> Paths; //storing all lenghts of paths of leaves.
...
public:
  ...
  {
    root = curr_root;
    shortestPathLength = longestPathLength = 1;
  }
  ...
  TreeNode *getSuccessor(TreeNode *root); //resturns the successor of node.
  TreeNode *getMin(TreeNode *root); // returns the minimum of root's subtree.

  // Returns the imbalance of the root node of the BST
  int getImbalance();               //returns the imbalance of tree.
  int Lenght(TreeNode *leaf);      //returns the length of path of leaf
  void InsertAllPaths();            //clears the paths vector and calls insertallleaflenghts.
  void InsertAllLeafLengths(TreeNode *root);//inserts all leaf lengths of the root(passed as an argument) into the vector paths.
