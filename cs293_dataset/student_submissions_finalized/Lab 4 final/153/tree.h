...
class BST{
  ...
  TreeNode* findAndReturn(int JourneyCode, int price);
  TreeNode* makeLeaf(int JourneyCode, int price, TreeNode* parent);
  bool removeNode(TreeNode* cur_node);
  bool isLeftChild(TreeNode* node);
  TreeNode* minNode(TreeNode* sub_root);
  void updatePathLengths(TreeNode* node);
