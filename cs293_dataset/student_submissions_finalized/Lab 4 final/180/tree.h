...
struct TreeNode{
  ...
  int get_short(TreeNode *n){
    if(n == NULL){
      return 0;
    }
    else{
      return 1 + min(get_short(n->left),get_short(n->right));
    }
  }

  int get_long(TreeNode *n){
    if(n == NULL){
      return 0;
    }
    else{
      return 1 + max(get_long(n->left),get_long(n->right));
    }
  }

  int getImbalance()
  {
    longestPathLength = get_long(this);
    shortestPathLength=get_short(this);
