...
struct TreeNode{
  ...
  TreeNode(unsigned int jrnyCode,unsigned int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr) {}
  
  TreeNode(unsigned int jrnyCode,unsigned int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent) {}
  int shortlength(TreeNode* d){
    if(d==nullptr) return 0;
    else{
        return(1+min(shortlength(d->left),shortlength(d->right)));
    }
  }
  int longlength(TreeNode* d){
    if(d==nullptr) return 0;
    else{
        return(1+max(longlength(d->left),longlength(d->right)));
    }
  }
  int getImbalance()
  {
    ...
    shortestPathLength=shortlength(this);
    longestPathLength=longlength(this);
...
public:
  ...
  // Constructors!
  BST() {root = nullptr;}
  BST(TreeNode *curr_root) {root = curr_root; root->getImbalance();}
