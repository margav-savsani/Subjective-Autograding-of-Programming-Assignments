...
bool Contains(int jrnycode, int val, TreeNode*& node) {
    if(node == nullptr){
        return false;
    } else if(jrnycode == node->JourneyCode){
        return true;
    } else if(jrnycode < node->JourneyCode){
...
bool Insert(int jrnycode, int val, TreeNode* node) {
    ...
    if(jrnycode < node->JourneyCode){
        if(node->left == nullptr){
            ...
            node->left->shortestPathLength=minheight(node->left);
            node->left->longestPathLength=maxheight(node->left);
            node->shortestPathLength=minheight(node);
            node->longestPathLength=maxheight(node);
            return true;
        } else{
        ...
        if(node->right == nullptr){
            ...
            node->right->shortestPathLength=minheight(node->right);
            node->right->longestPathLength=maxheight(node->right);
            node->shortestPathLength=minheight(node);
            node->longestPathLength=maxheight(node);
            return true;
        } else{
