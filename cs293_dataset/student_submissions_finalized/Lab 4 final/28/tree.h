...
struct TreeNode{
  ...
  int shortestPathLength=shortpath(); // Length of shortest path from this node to a leaf
                          // where a nullptr leaf contributes a path of length 0
  int longestPathLength=longpath(); // Length of longest path from this node to a leaf
                         // where a nullptr leaf contributes a path of length 0
  ...
  int longpath(){
    ...
    else if(left==NULL) return 1+right->longpath();
    else if(right==NULL) return 1+left->longpath();
    else {
      if (right->longpath()>=left->longpath()) return 1+right->longpath();
      else return 1+left->longpath();
    }
  }
  
  int shortpath(){
    if(left==NULL&&right==NULL) return 0;
    else if(left==NULL) return 0;
    else if(right==NULL) return 0;
    else {
      if (right->shortpath()<=left->shortpath()) return 1+right->shortpath();
      else return 1+left->shortpath();
    }
  }
