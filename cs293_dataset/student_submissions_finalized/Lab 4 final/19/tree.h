...
struct TreeNode{
  ...
  int maxh(TreeNode * root){
    if (root == NULL)
      return 0;
    int leftDepth = maxh(root->left);
    int rightDepth = maxh(root->right);
    if (leftDepth > rightDepth)
      return leftDepth + 1;
    else
      return rightDepth + 1;
  }
  int minh(TreeNode *root, int level){
    if (root == NULL)
      return level;
      level++;
    return min(minh(root->left, level), minh(root->right, level));
  }
...
public:
  ...
  int maxDepth(){
    return root->maxh(root);
  }
  int minDepth(){
    return root->minh(root, 0);
  }
