...
struct TreeNode{
  ...
  int shortest() {
    ...
    return 1+std::min(left->shortest(),right->shortest());
  }
  int longest() {
    if(left==NULL && right==NULL) return 1;
    return 1+std::max(left->shortest(),right->shortest());
  }
  void assign() {
...
public:
  ...
  // Constructors!
  BST() {root = nullptr;} // shortestPathLength =0; root->longestPathLength = 0;}
  BST(TreeNode *curr_root) {root = curr_root; root->shortestPathLength = 1;root->longestPathLength = 1;}
  ...
  TreeNode* find_tree(TreeNode* x, int price);
