...
struct TreeNode{
	...
	bool insert_in_subtree(int jcode, int pr){
		if(price >= pr){
			if(!left){
				left = new TreeNode(jcode, pr, NULL, NULL, this);
				left->update_max_dist();
				left->update_min_dist();
			} 
			else left->insert_in_subtree(jcode, pr);
		}
		else if(price < pr){
			if(!right){
				right = new TreeNode(jcode, pr, NULL, NULL, this);
				right->update_max_dist();
				right->update_min_dist();
			}
