...
struct TreeNode{
  ...
  void updateLengths(){ //Max_Length = 1 + max(left->Max_Length, right->Max_Length)
    ...
    if(left == nullptr){
      ...
      min_left = left->shortestPathLength;
      max_left = left->longestPathLength;
    }
    ...
    if(right == nullptr){
      ...
      min_right = right->shortestPathLength;
      max_right = right->longestPathLength;
    }

    shortestPathLength = 1 + min(min_left, min_right);
    longestPathLength = 1 + max(max_left, max_right);
  }
...
class BST{
  ...
  void deleteNode(TreeNode* node_ptr);
...
public:
  ...
  bool find(int JourneyCode, int price, TreeNode *node_ptr);
