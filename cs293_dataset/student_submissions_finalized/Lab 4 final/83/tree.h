...
struct TreeNode
{
  ...
  int getImbalance()
  {
    ...
    longestPathLength = TreeNode::shortpath(root);
    shortestPathLength = TreeNode::longpath(root);s
  ...
  int shortpath(TreeNode *root)
  {
    ...
    if (root->left == nullptr)
      return 1 + BST::shortpath(root->right);
    else if (root->right == nullptr)
      return 1 + BST::shortpath(root->left);
    else
    {
      int minleft = BST::shortpath(root->left);
      int minright = BST::shortpath(root->right);
  ...
  int longpath(TreeNode *root)
  {
    if (root == NULL)
      return 0;
    int maxleft = longpath(root->left);
    int maxright = longpath(root->right);
