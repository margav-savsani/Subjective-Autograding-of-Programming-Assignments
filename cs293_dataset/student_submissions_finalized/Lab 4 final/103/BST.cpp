...
int BST::countJourneysInPriceBoundHelper(int  lowerPriceBound, int upperPriceBound, TreeNode* cnode){
    ...
    if(lowerPriceBound <= cnode->price && cnode->price <= upperPriceBound){
...
void BST::loadBSTList(vector< pair<int, int> > list){
    ...
    root->parent = NULL;
    int idx = 0;
    loadTraverse(list, idx, root);
}

// Calcuates the path lengths of all nodes in the BST
void BST::calculatePathLengths(){
    calculatePathLengthsHelper(root);
}

// Helper function to calculatePathLengths()
void BST::calculatePathLengthsHelper(TreeNode* cnode){
    if(cnode == NULL){
        return;
    }

    calculatePathLengthsHelper(cnode->left);
    calculatePathLengthsHelper(cnode->right);

    if(cnode->left == NULL && cnode->right == NULL){
        cnode->longestPathLength = 1;
        cnode->shortestPathLength = 1;
    } else if(cnode->left == NULL){
        cnode->longestPathLength = 1 + cnode->right->longestPathLength;
        cnode->shortestPathLength = 1 + cnode->right->shortestPathLength;
    } else if(cnode->right == NULL){
        cnode->longestPathLength = 1 + cnode->left->longestPathLength;
        cnode->shortestPathLength = 1 + cnode->left->shortestPathLength;
    } else{
        cnode->longestPathLength = 1 + max(cnode->left->longestPathLength, cnode->right->longestPathLength);
        cnode->shortestPathLength = 1 + min(cnode->left->shortestPathLength, cnode->right->shortestPathLength);
    }
}

// Returs the imbalance of the BST
int BST::getImbalance(){
    return root->longestPathLength - root->shortestPathLength;
}
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    return countJourneysInPriceBoundHelper(lowerPriceBound, upperPriceBound, root);
}
...
void BST::customStore(string filename) {
    ...
    vector< pair<int, int> >list = saveBSTList();
    for(int i = 0; i < list.size(); i++){
        outfile << list[i].first << " " << list[i].second << endl;
    }
...
void BST::customLoad (int flag, string filename) { 
    ...
    while(infile >> price){
        ...
        if(flag == LOAD_FULL_BST){
            list.push_back({price, JourneyCode});
