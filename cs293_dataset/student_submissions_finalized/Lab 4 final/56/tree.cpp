...
int BST::getImbalance()  //return root's imbalance
{
...
bool BST::insert(int JourneyCode, int price)
{
  ...
  bool res;       //bool to store the exit code of insert
    if(root != NULL){        
        if(JourneyCode < root->JourneyCode){              //insert in left subtree
        ...
        else if(JourneyCode >= root->JourneyCode){   //insert in the right subtree
        
            if(root->price == price && root->JourneyCode == JourneyCode){    //if equal, => same entry => reject
                res = false;
