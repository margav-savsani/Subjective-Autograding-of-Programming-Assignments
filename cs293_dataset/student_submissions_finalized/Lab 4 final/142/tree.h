...
struct TreeNode{
  ...
  int countpathLength(TreeNode *root){
    int count=0;
    while(true){
      if(root!=nullptr){
        count++;
        if(root->left!=nullptr)countpathLength(root->left);
        if(root->right!=nullptr)countpathLength(root->right); 
      }
      else break;
        /*{
        AllpathLengths[i]=count;
        i++;
        continue;
      }
      if(end of BST)break;*/
  ...
  int getImbalance()
  {
    int longestPathLength, shortestPathLength;
    TreeNode *root;
    int count=0,i=0,*AllpathLengths;
    AllpathLengths[i]=countpathLength();
  /*  while(true){
      if(!root==nullptr){
      count++;
      }
    ...
    shortestPathLength = min(AllpathLengths);
    longestPathLength = max(AllpathLengths);
...
public:
  ...
  BST(TreeNode *curr_root) {root = curr_root; curr_root->shortestPathLength = curr_root->longestPathLength = 1;}
