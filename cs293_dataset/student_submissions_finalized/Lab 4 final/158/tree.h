...
struct TreeNode{
  ...
  int shortestPathLength=999999; // Length of shortest path from this node to a leaf
...
class BST{
  ...
  // Returns false, if given JourneyCode is already present in the BST
	...
	Journey  inOrderSuccessor(TreeNode* l);
	TreeNode* remove(int JourneyCode, int price, TreeNode* l);
	...
	//Generates predecessor of given TreenODE;
	struct TreeNode* predecessor(struct TreeNode* node);

	//Helper func
	bool journeyCheck(struct TreeNode *l, int JourneyCode);

	
	
	
	//Ordering Functions:
	void preorder(struct TreeNode *l);
	void postorder(struct TreeNode *l);
	void inorder(struct TreeNode *l);
	...
	// Returns the count of journeys that are at least as expensive
        ...
        //sub-function for Ordered Bound Prices:
	int inorderBoundCount(int lowerBound, int upperBound, struct TreeNode* l);
	...
	// Store the current tree in a file called filename.  You should
        ...
        void helpStore(string filename, struct TreeNode* l);
