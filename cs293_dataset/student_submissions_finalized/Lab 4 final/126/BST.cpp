...
bool BST::insert(int JourneyCode, int price) {
    ...
    else if((y->JourneyCode)>JourneyCode)
        y->left=ins;
    else
        y->right=ins;
    ins->length=length;
...
void BST::pre1(Journey *n, ostream& outfile){//function for printing the tree using sentinels
    if(n==nullptr){
        return;
    }
    else{
...
int BST::count(Journey *n, int low, int up){//counts the number of journeys within price range
    if (n==nullptr){
        return 0;
    }
    if(n->price>up){
...
void BST::imbalance(Journey *n, int &m){
    if(n->left==nullptr&&n->right==nullptr){
        if(n->length<m){
            short1=m;
        }
            ...
            imbalance(n->right, short1);
    }
    else if(n->right==nullptr){
            imbalance(n->left, short1);
    }
    else{
        imbalance(n->right, short1);
        imbalance(n->left, short1);
