...
struct TreeNode{
  ...
  int l_P(TreeNode *root){
	  if ( root != NULL ){
		  return 1+max(l_P(root->right, l_P(root->left));
	  }
	  else return 0;
  }

  int s_p(TreeNode *root){
  	if if ( root != NULL ){   
                  return 1+min(s_P(root->right, s_P(root->left));
          }
          else return 0;
}
  int getImbalance()
  {
  longestPathLength = 1+max(l_P(left),l_P(right));
  shortestPathLength = 1+min(s_P(left),s_P(right));
