...
bool BST::insert(int JourneyCode, int price)
{
  ...
  currNode=new TreeNode(JourneyCode, price, nullptr, nullptr, parentOfCurrNode);
    ...
    root->shortestPathLength=spl(root);
    root->longestPathLength=lpl(root);
...
int BST::lpl(TreeNode* node){
  if (node == NULL)
        return -1;
    else
    {
        // compute the depth of each subtree 
        int lp = lpl(node->left);
        int rp = lpl(node->right);
