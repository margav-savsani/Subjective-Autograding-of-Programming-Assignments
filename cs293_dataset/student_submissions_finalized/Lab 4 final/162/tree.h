...
struct TreeNode{
  ...
  TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent) {}
   void setMaxDepth(TreeNode* &node)
  {
      if(node== NULL) return;
      ...
      setMaxDepth(node->left);
      setMaxDepth(node->right);
      int leftDepth=0,rightDepth=0;
      if(node->left != nullptr)
        leftDepth = node->left->longestPathLength;
      if(node->right != nullptr)
        rightDepth = node->right->longestPathLength;
      node->longestPathLength =  max(rightDepth,leftDepth) +1;
  }

  void setMinDepth(TreeNode* &node)
  {
      if(node== NULL) return;
      if(node->left == NULL && this->right == NULL) node->shortestPathLength  = 1;
      setMinDepth(node->left);
      setMinDepth(node->right);
      int leftDepth=0,rightDepth=0;
      if(node->left != nullptr)
        leftDepth = node->left->shortestPathLength;
      if(node->right != nullptr)
         rightDepth = node->right->shortestPathLength;
      node->shortestPathLength=  min(rightDepth,leftDepth) +1;
  }
