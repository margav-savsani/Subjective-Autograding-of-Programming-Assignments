...
//
struct TreeNode;
int longesth(TreeNode *node);
int shortesth(TreeNode *node);
struct TreeNode{
  ...
  int getImbalance()
  {
    ...
    int kr=longesth(right);
    int kl=longesth(left);
    longestPathLength=(std::max(kr,kl))+1;
    kr=shortesth(right);
    kl=shortesth(left);
    shortestPathLength=(std::min(kr,kl))+1;
    return (longestPathLength - shortestPathLength);  //imbalance is longest-shortest
  }
    
};
int longesth(TreeNode *node)    //to get the longest path
{
  int height=0;
  if (node!=nullptr)
  {
    int l_height=longesth(node->left);
    int r_height=longesth(node->right);
    int max_height=std::max(l_height,r_height);//height is 1+max(childs)
    height=max_height+1;
  }
  return height;  //return height
}
int shortesth(TreeNode *node) //to get the min
{
  int height=0;
  if (node!=nullptr)
  {
    int l_height=shortesth(node->left);
    int r_height=shortesth(node->right);
    int min_height=std::min(l_height,r_height);//min heigth is 1+min(childs)
    height=min_height+1;
  }
  return height;
}
...
public:
  ...
  TreeNode *getSuccessor(int JourneyCode,int price);
