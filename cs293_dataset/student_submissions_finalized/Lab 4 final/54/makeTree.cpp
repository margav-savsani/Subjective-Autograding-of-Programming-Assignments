...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        else if (parse[0] == "sortinsert")
        {   T.QuicksortWithBSTInsert(stoi(parse[1]), stoi(parse[2]));
            if (T.QuicksortWithBSTInsert(stoi(parse[1]), stoi(parse[2])))
            {
                cout<< "QUICK SORTED WITH INSERTING BST" <<endl;
            }
            else
                cout << "QUICK SORT WITH INSERT BST FAILED" << endl;
        }
        ...
        {  
         ...
         else if (parse[0]=="printimbalance")
         {
            cout<<T.imbalance()<<endl;
         }
         else 
