...
struct TreeNode{
  ...
  friend bool operator<(const TreeNode& leftArgument, const TreeNode& rightArgument)
  {
    return (leftArgument.JourneyCode < rightArgument.JourneyCode);
  }
  friend bool operator<=(const TreeNode& leftArgument, const TreeNode& rightArgument)
  {
    return (leftArgument.JourneyCode <= rightArgument.JourneyCode);
...
public:
  ...
  // Constructors!
  BST() {root = nullptr; shortestPathLength = longestPathLength = 0;}
  BST(TreeNode *curr_root) {root = curr_root; shortestPathLength = longestPathLength = 1;}
  ...
  bool find(int JourneyCode, int price);
     ...
     TreeNode *Minelem(TreeNode *a)
        { // returns the Node containing the minimum key value
          // Takes the argument as root of the tree.
                while (a->left != NULL)
   ...
   TreeNode *successor(TreeNode *given)
        { // returns the successor of the given node.
                if (given->right != NULL)
                { // if there is a right child of the given node
                  // Then the successor is min of the right subtree.
                        return Minelem(given->right);
                }
                // Otherwise
                TreeNode *y = given->parent; // y is the parent of the given node
                while (y != NULL && (given == y->right))
                {
                        given = y;
                        y = y->parent;
                }
  ...
  int getImbalance();
  int maxhig(TreeNode *base){
    if (base == nullptr) return 0;
    int lhig;int rhig;
    lhig = maxhig(base->left);
    rhig = maxhig(base->right);
    if(lhig < rhig) return (rhig+1);
    else return (lhig+1);
  }
int minhig(TreeNode *base) {
  if (base == nullptr) return 0;
    int lhig = minhig(base->left);
    int rhig = minhig(base->right);
    if(lhig < rhig) return (lhig+1);
    else return (rhig+1);
};
