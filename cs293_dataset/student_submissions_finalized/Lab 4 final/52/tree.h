...
struct TreeNode{
  ...
  // Updates shortest and longest path lengths using children data
  bool updatePathLengths(){
    int ls=-1, rs=-1, ll=-1, rl=-1;
    if(left != nullptr){
      ls = left->shortestPathLength;
      ll = left->longestPathLength;
    }
    if(right != nullptr){
      rs = right->shortestPathLength;
      rl = right->longestPathLength;
    }

    int s = min(ls, rs) + 1;
    int l = max(ll, rl) + 1;

    if(s == shortestPathLength && l == longestPathLength) return false;
    else{
