...
struct TreeNode
{
  ...
  TreeNode() : price(0), JourneyCode(0), left(nullptr), right(nullptr), parent(nullptr), shortestPathLength(1), longestPathLength(1) {}

  TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr), parent(nullptr), shortestPathLength(1), longestPathLength(1) {}
...
public:
  ...
  // Returns false, if given JourneyCode is already present in the BST
  // Inserts the element and returns True otherwise
  bool myInsert(int JourneyCode, int price, TreeNode *insertInMe);

  // Returns true iff given journey code is present in given subtree
