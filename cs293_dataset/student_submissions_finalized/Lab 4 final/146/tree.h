...
struct TreeNode{
  ...
  bool operator<( const TreeNode& Argument) // $ removed two inputs
  {
    return (JourneyCode < Argument.JourneyCode);
  }
...
class BST{
  ...
  int shortestPathLength,longestPathLength; // $ Added missing variables 
...
public:
  ...
  //Updates the values of shortest and longest path lengths of all nodes
  bool updateLengths(TreeNode* root);
