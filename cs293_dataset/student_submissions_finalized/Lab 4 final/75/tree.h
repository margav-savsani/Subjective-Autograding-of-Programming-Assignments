...
struct TreeNode{
  ...
  // Updates the longest path length from the node to the leaf
  // NOTE: THE NODE ONLY STORES THE LONGEST PATH FROM THAT NODE TO LEAF
  // BECAUSE OF THE PROBLEM OF MUTIPLE SHORTEST PLACES IT IS DIFFICULT TO 
  // KEEP TRACK OF WHICH PATH REMAINS SHORTEST ONCE ONE OF THE PATH IS UPDATED
  // WHICH CAN POSSIBLY TAKE O(N) time
  // void updateLongestPath(int index){
  //   if (index>longestPathLength){
  //     longestPathLength = index;
  //   }
  // }
  int getLongestPath(){
    if (left!=NULL && right!=NULL){
      return (1 + max(left->longestPathLength, right->longestPathLength));
    }
    else if(left!=NULL){ // right child is NULL
      return (1 + left->longestPathLength);
    }
    else if(right!=NULL){ // left child is NULL
      return (1 + right->longestPathLength);
    }
  ...
  int getShortestPath(){
    if (left!=NULL && right!=NULL){
      return (1 + min(left->shortestPathLength, right->shortestPathLength));
    }
    else if(left!=NULL){ // right child is NULL
      return (1 + left->shortestPathLength);
    }
    else if(right!=NULL){ // left child is NULL
      return (1 + right->shortestPathLength);
    }
    else{ // the node is leaf
      return 0;
    }
