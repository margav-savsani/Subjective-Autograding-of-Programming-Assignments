...
struct TreeNode{
  ...
  int getImbalance()
  {
    ...
    return (getMax(this) - getMin(this));
  }

  int getMax(TreeNode *v){
    if (v==NULL) return -1;
    else return 1 + max(getMax(v->left),getMax(v->right));
  }

  int getMin(TreeNode *v){
    if (v==NULL) return -1;
    else return 1 + min(getMin(v->left),getMin(v->right));
  }
};
...
public:
  ...
  // search the closest leaf downwards, and return the MAX and MIN lengths
  int searchDownMAX(TreeNode *v);
  int searchDownMIN(TreeNode *v);
