...
struct TreeNode{
  ...
  int getImbalance()
  {  longestPathLength = mylong(this);
     shortestPathLength = myshort(this);
  ...
  int myshort(TreeNode *a){
    if (a == NULL) {return -1;}
    else {
       return 1+min(myshort(a->left),myshort(a->right));
    }
  }
   int mylong(TreeNode *a){
    if (a == NULL) {return -1;}
    else {
       return 1+max(mylong(a->left),mylong(a->right));
    }
  }
