...
struct TreeNode{
  ...
  if(A == NULL) {return 0;}
     ...
     return min((1+shortpath(A->left)),(1+shortpath(A->right)));
  }
}
int longpath(TreeNode *A){
  ...
  if(A == NULL) {return 0;}
    ...
    return max((1+longpath(A->left)),(1+longpath(A->right)));
  }
}

  int getImbalance()
  {
    shortestPathLength = shortpath(this);
    longestPathLength = longpath(this);
