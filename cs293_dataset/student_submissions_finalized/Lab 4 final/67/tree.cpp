...
int TreeNode::shortestPathLength(){
	if(!left || !right){ //0 is the shortest length
		return 0;
	}
	//depends on smallest path among subtrees
	int a1=left->shortestPathLength();
	int a2=right->shortestPathLength();
	if(a1<a2)	return a1+1;
	return a2+1;
}
int TreeNode::longestPathLength(){
	if(!left){
		if(!right){ // 0 iff no child
			return 0;
		}
		return right->longestPathLength()+1; //the longer one to be returned
	}
	if(!right)	return left->longestPathLength()+1;
	int a1=left->longestPathLength();
	int a2=right->longestPathLength();
	if(a1>a2)	return a1+1;
	return a2+1;
}
...
bool BST::insert(int JourneyCode, int price){
	...
	while(curr){
		...
		if(JourneyCode<=curr->JourneyCode && curr->left){//if insertion is to be done on left
			curr=curr->left;
			continue;
		}
		if(JourneyCode>curr->JourneyCode && curr->right){// if insertion is to be done on right
			curr=curr->right;
			continue;
		}
...
int BST::getMinimum() {
	return getMinimum(root)->JourneyCode;
}
