...
struct TreeNode{
	...
	int longestPathLength(); //Returns the longest path length from this node to a leaf
	int shortestPathLength(); //Returns the shortest path length from this node to a leaf
...
class BST{
	...
	// Return node with minimum price in the tree following node
	TreeNode* getMinimum(TreeNode* node);
	
	// Return node with maximum price in the tree following node
...
public:
	...
	// Return the first node with given price
	// Any node with the given price will be to left of it
	TreeNode* search_node(int price);
