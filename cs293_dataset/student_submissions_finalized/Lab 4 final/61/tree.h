...
struct TreeNode{
  ...
  int longestpath(){
    if(left==NULL && right==NULL){
      ...
      return 1+right->longestpath();
    }  
    else if(right==NULL){
      return 1+left->longestpath();
    }  
    else{
      return 1+max(left->longestpath(),right->longestpath());
    }  
  }
  int shortestpath(){
    if(left==NULL && right==NULL){
      ...
      return  1+right->shortestpath();
    }  
    else if(right==NULL){
      return  1+left->shortestpath();
    }  
    else{
      return 1+min(left->shortestpath(),right->shortestpath());
    }  
  }
};
 void traverse_pre(TreeNode* root){
        ...
        cout<<x->price<<" ";
        traverse_pre(x->left);
        traverse_pre(x->right);
 }
 void traverse_io(TreeNode* root){
        TreeNode* x=root;
        if(x==NULL){
                return;
        }
  ...
  void loading(TreeNode* root, ofstream &outfile){
        TreeNode* x=root;
        if(x==NULL){
                return;
        }
        ...
        outfile<<x->price<<" "<<x->JourneyCode<<" ";
        loading(x->left,outfile);
        loading(x->right,outfile);
 }
 void traverse_post(TreeNode* root){
        ...
        traverse_post(x->left);
        traverse_post(x->right);
        cout<<x->price<<" ";
 }
 void inorder(TreeNode* root, int &a , int lowerPriceBound, int upperPriceBound ){
        TreeNode* x=root;
        if(x==NULL){
                return;
        }
        inorder(x->left,a,lowerPriceBound,upperPriceBound);
        if(x->price>=lowerPriceBound && x->price<=upperPriceBound){
            a++;
        }
        inorder(x->right,a,lowerPriceBound,upperPriceBound);
 }
class BST{
