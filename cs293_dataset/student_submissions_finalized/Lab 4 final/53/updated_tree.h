...
// Reuse the binary Search Tree implemented in lab 3 as much as possible->done
...
struct TreeNode{
  ...
  TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent) {
    if(leftChild==nullptr && rightChild==nullptr) longestPathLength=1;
    else if(leftChild!=nullptr && rightChild==nullptr) longestPathLength= rightChild->longestPathLength + 1;
    else if(rightChild!=nullptr && leftChild==nullptr) longestPathLength= leftChild->longestPathLength + 1;
    else longestPathLength = max(rightChild->longestPathLength,leftChild->longestPathLength)+1;
  ...
  int getImbalance()
  {
    ...
    // Think about when would we have a case of large imbalance->ans->when we have inputs in a sorted way or reverse sorted way
...
public:
  ...
  // Constructors!
  BST() {root = new TreeNode(); //initialising the basic private variables 
    root->parent = nullptr;
  ...
  //updates longestpathlength for parent nodes during insertion or deletion of a new node
  void update_longest_path_length(TreeNode* z); 

  //updates shortestpathlength for parent nodes during insertion or deletion of a new node
  
  void update_shortest_path_length(TreeNode* z);

  //Returns the pointer to the TreeNode after searching or nullptr if search is unsuccessful(added by me) 
  TreeNode* search(int JourneyCode, int price);

  //Returns the pointer to the successor of the given pointer to the TreeNode(added by me)
