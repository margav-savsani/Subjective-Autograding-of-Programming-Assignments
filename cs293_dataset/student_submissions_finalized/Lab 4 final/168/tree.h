...
struct TreeNode{
    ...
    int shortestPathLength=1; // Length of shortest path from this node to a leaf
                          // where a nullptr leaf contributes a path of length 0
    int longestPathLength=1; // Length of longest path from this node to a leaf
                         // where a nullptr leaf contributes a path of length 0
    // Constructors
...
public:
	...
	bool find(int JourneyCode, int price);
        ...
        //printing the shortest and longest path from that node to a leaf for all
        //nodes in the tree.
        void prestore();
