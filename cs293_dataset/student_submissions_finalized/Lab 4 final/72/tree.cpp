...
bool BST::insert(int JourneyCode, int price)
{
  ...
  if(parentOfCurrNode != nullptr){
    if(parentOfCurrNode->JourneyCode < JourneyCode)parentOfCurrNode->right=z;
  ...
  TreeNode *curr=z;
  TreeNode *par=z->parent;
  while(par != nullptr){
    if(par->right == nullptr || par->left == nullptr){
      par->shortestPathLength=1;
      par->longestPathLength=curr->longestPathLength+1;
    }
    else{
      if(par->left->shortestPathLength < par->right->shortestPathLength){
        par->shortestPathLength = par->left->shortestPathLength + 1;
      }
      else{
        par->shortestPathLength = par->right->shortestPathLength + 1;
      }
      if(par->left->longestPathLength < par->right->longestPathLength){
...
bool BST::find(int JourneyCode, int price)
{
  ...
  while(currNode != nullptr){
    if(currNode->JourneyCode == JourneyCode)return true;
