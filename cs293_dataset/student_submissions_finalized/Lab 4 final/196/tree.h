...
struct TreeNode{
  ...
  TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode),parent(nullptr), left(nullptr), right(nullptr),shortestPathLength(0),longestPathLength(0){}
  ...
  TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent) {
    if (left==nullptr && right==nullptr){
      ...
      shortestPathLength=min(left->shortestPathLength+1,right->shortestPathLength+1);
      longestPathLength=max(left->longestPathLength+1,right->longestPathLength+1);
    }
  }
