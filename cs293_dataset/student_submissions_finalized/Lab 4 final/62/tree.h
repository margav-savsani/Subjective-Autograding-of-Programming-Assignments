...
struct TreeNode{
  ...
  //following definition altered to make more sense for imbalance
  int shortestPathLength; // Length of shortest path to a null node
                          // where a nullptr leaf contributes a path of length 0
  int longestPathLength; // Length of longest path to a null node
                         // where a nullptr leaf contributes a path of length 0
  ...
  {
    return (this->JourneyCode < rightArgument.JourneyCode);
  }
