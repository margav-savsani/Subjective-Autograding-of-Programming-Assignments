...
struct TreeNode{
  ...
  TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr),parent(nullptr) {price=jrnyPrice;JourneyCode=jrnyCode;shortestPathLength=1;longestPathLength=1;}
  ...
  int getImbalance()
  {
    ...
    cout<<longestPathLength<<shortestPathLength;
...
public:
  ...
  int checks(int j,TreeNode *y);
  int checkl(int j,TreeNode *y);
