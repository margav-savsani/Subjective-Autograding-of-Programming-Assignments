...
struct TreeNode{
  ...
  int getLongest(TreeNode *x){
    if(x==NULL){return 0;}
    else if(x->left==NULL && x->right==NULL){return 1;}
    else if(x->left==NULL && x->right!=NULL){return 1+getLongest(x->right);}
    else if(x->right==NULL && x->left!=NULL){return 1+getLongest(x->left);}
    else if(x->left!=NULL && x->right!=NULL){
      if(getLongest(x->right)<getLongest(x->left)){return 1+getLongest(x->left);}
      else{return 1+getLongest(x->right);}
  }}
  int getSmallest(TreeNode *x){
    if(x==NULL){return 0;}
    else if(x->left==NULL && x->right==NULL){return 1;}
    else if(x->left==NULL && x->right!=NULL){return 1+getSmallest(x->right);}
    else if(x->right==NULL && x->left!=NULL){return 1+getSmallest(x->left);}
    else if(x->left!=NULL && x->right!=NULL){
      if(getSmallest(x->right)<getSmallest(x->left)){return 1+getSmallest(x->right);}
      else{return 1+getSmallest(x->left);}
    
  }}
  int getImbalance()
  {longestPathLength=getLongest(this);
    shortestPathLength=getSmallest(this);
...
void changeroot(TreeNode *x){root=x;}
