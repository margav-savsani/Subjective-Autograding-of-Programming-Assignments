...
struct TreeNode{
  ...
  /*
  	I'll just modify the constructors so that on initalizing the node, shortestPath=longestPath=0;
  */
  
  TreeNode() : price(0), JourneyCode(0), left(nullptr), right(nullptr), shortestPathLength(0),
               longestPathLength(0) {}
  
  TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr), 
                                          shortestPathLength(0), longestPathLength(0) {}
