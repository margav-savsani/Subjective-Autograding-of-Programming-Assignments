...
struct TreeNode
{
  ...
  TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr), longestPathLength(1), shortestPathLength(1) {}
...
public:
  ...
  int updateShortestPath(TreeNode* node);
  int updateLongestPath(TreeNode* node);
