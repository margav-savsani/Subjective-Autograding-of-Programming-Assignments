...
class Journey {
 ...
 public:
  ...
  Journey(unsigned int c, unsigned int p, bool rand) {JourneyCode=c; price=p;}
  
  unsigned int getJourneyCode(); // returns journey code
  unsigned int getPrice(); // returns price

  unsigned int getJourneyCodeNew();
  unsigned int getPriceNew();

  void setJourneyCodeNew(int c) {JourneyCode=c;}
  void setPriceNew(int p) {price=p;}
