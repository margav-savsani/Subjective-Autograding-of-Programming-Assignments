...
int maxLength(TreeNode * root){
	...
	return max(maxLength(root->right),maxLength(root->left))+1;
}

int minLength(TreeNode * root){
	if (root == nullptr) return 0;
	return min(minLength(root->right),minLength(root->left))+1;
}
...
bool BST::insert(int JourneyCode, int price)
{
	...
	root->shortestPathLength = minLength(root);
	root->longestPathLength = maxLength(root);
	return true;
}
...
bool BST::remove(int JourneyCode, int price)
{
    if (root->JourneyCode == JourneyCode && root->price == price){
		if(root->left == nullptr){
			...
			root->price = pre->price;
			root->JourneyCode = pre->JourneyCode;
		}
		return true;
    }
    else if (root->JourneyCode > JourneyCode && root->left != NULL) return BST(root->left).remove(JourneyCode, price);
    else if (root->right != NULL) return BST(root->right).remove(JourneyCode, price);
    return false;
}
// ************************************************************
