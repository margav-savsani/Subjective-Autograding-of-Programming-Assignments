...
struct TreeNode{
  ...
  TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent){
            JourneyCode=jrnyCode;price=jrnyPrice;right=rightChild;left=leftChild;parent=parent; 
  }
  int getImbalance()
  {
...
public:
  ...
  int shortpath(TreeNode* a);
  int longpath(TreeNode* a);
  void update(TreeNode* k);
  bool remove(int JourneyCode, int price);
  TreeNode* successor(TreeNode* k);
  TreeNode* minimum(TreeNode* k);
