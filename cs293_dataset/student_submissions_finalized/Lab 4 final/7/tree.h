...
int longest(TreeNode* n){
  struct TreeNode t;
  if(n == nullptr){
    return 0;
  }
  
  leftTreeHeight = longest(n->left);
  rightTreeHeight = longest(n->right);
  
  return (max(leftTreeHeight,rightTreeHeight)+1);
  
}

int shortest(TreeNode* n){
	if(n == nullptr){
		return 0;
	}
	
	leftTreeHeight = shortest(n->left);
	rightTreeHeight = shortest(n->right);
	
	return ( min(leftTreeHeight,rightTreeHeight) + 1);
  ...
  int getImbalance()
  {
    ...
    int longestPathLength=longest(*this);
    int shortestPathLength = shortest(*this);
