...
public:
  ...
  BST(TreeNode *curr_root) {root = curr_root; root->shortestPathLength = root->longestPathLength = 0;}

  // Implement your own destructor
  ~BST() {};

  // gives shortestPathLength
  int getshortpath(TreeNode *node);

  // gives longestPathLength
  int getlongpath(TreeNode *node);
