...
int Dictionary :: hashValue(char* key){
    ...
    for(int i=0; i<32; i++){
        if(key[i]==0)
            break;
        for(int j=0; j<i; j++)
            key[i] = (key[i]*poly_x)%100003; //polynomial methond // 100003 is some random 6digit prime
        keyvalue = (keyvalue + key[i])%100003;
    }
...
bool Dictionary :: get(char key[], float time, Entry *elelist, int *len){
    *len = 0;
    int N = DICT_SIZE;
    int index = hashValue(key);   // starting point of search
    for(int i=index; i<64+index; i++){
        ...
        if(strcmp(key, A[i%N].value->init_station)==0){  //complare two char lists
            if(A[i%N].value->init_time >= time){
                elelist[*len]=A[i%N];
                *len += 1;
            }
        }
