...
class Planner {
  ...
  void addJourney(Journey j){
    ...
    if(ans==NULL){
      ...
      int index=ans->value;
      float time=j.start_time;
      routenode* r= &route[index];
      routenode* s= new routenode;
      if(r->current.start_time>=time){
        s->current=r->current;\
        s->next=r->next;
        r->current=j;
        r->next=s;
        ...
        if(time<=r->next->current.start_time){
          s->next=r->next;
          r->next=s;
          break;
        }
  ...
  void query_station(q_station q){
    ...
    if(ans==NULL){
      ...
      int total=0;
      while(r!=NULL){
        if(r->current.start_time>=q.time){
          total++;
          cout<<endl<<r->current.start_time<<" "<<r->current.end.name;
        }
      ...
      while(r!=NULL){
        ...
        if(r->current.start_time>=q.time){
          if(strcmp(r->current.end.name , q.end.name)==0){
              ...
              cout<<endl<<r->current.start_time;
              return;
          }
          else{
            Entry* ans2;
            ...
            if(ans2==NULL){}
              ...
              routenode* x=&route[i];
              while(x!=NULL){
                if((x->current.start_time >= r->current.end_time)&& (strcmp(x->current.end.name,q.end.name)==0)){
                  cout<<endl<<r->current.start_time<<" "<<r->current.end.name<<" "<<(x->current.start_time);
                  return;
                }
                x=x->next;
              }
