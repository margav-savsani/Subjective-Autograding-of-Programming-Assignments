...
{
    N = 0;
    A = new Journey[DICT_SIZE]; // Creates the array of struct Entry of size DICT_SIZE.
    for (int i = 0; i < DICT_SIZE; i++)
    {
...
struct Journey *Dictionary::get(char key[])
{
    ...
    Journey* B=new Journey [DICT_SIZE];
    int j=0;
    for (int i = 0; i < DICT_SIZE; i++)
    { 
        ...
        { 
          B[j]=A[val];
          B[j].start_time = A[val].start_time;
          B[j].finish_time = A[val].finish_time;
          strcpy(A[val].key, B[j].key);
          strcpy(A[val].station_two, B[j].station_two);
          j++;
        }
...
int Dictionary::getTrains(char key[]){
    int val = hashValue(key);
    int j=0;
    for (int i = 0; i < DICT_SIZE; i++)
    { 
...
bool Dictionary::put(struct Journey e)
{
    ...
    A[val] = e;
    A[val].start_time = e.start_time;
    A[val].finish_time = e.finish_time;
    strcpy(e.key, A[val].key);
    strcpy(e.station_two, A[val].station_two); // Copies value of e.key to A[val].key
    if (val == -1)
    {
        return false;
    }
