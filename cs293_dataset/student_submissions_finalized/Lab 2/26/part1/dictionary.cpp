...
void Dictionary::gettrain(char key[], float time)
{
...
void Dictionary::journey(char key[], float time, char fin[])
{
    ...
    for (int i = 0; i < 64; i++)
    {
        if (strcmp(A[id].key, key) == 0)
        {
            if ((A[id].start >= time) && (strcmp(A[id].s2, fin) == 0))
            {
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        if (strcmp(A[id3].key, key) == 0)
        {
            if (A[id3].start >= time)
            {
                int id2 = hashValue(A[id3].s2);
                

                for (int i = 0; i < 64; i++)
                {
                    if (strcmp(A[id2].key,A[id3].s2) == 0)
                    {
                        if ((A[id2].start >= A[id3].end) && (strcmp(A[id2].s2, fin) == 0))
                        {
                            if (A[id3].start < first3)
                            {
                                first2 = A[id2].start;
                                place2 = id2;
                                first3=A[id3].start;
                            }
                            else if(A[id3].start==first3){
