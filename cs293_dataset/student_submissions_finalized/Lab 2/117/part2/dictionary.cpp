...
struct Entry* Dictionary::getAll(char key []){// returns the pointer of an element given its key, returns NULL if element not present
    int index = hashValue(key);// find hash value
    struct Entry* ans = NULL;
    struct Entry* tempArr = new struct Entry[DICT_SIZE];
    int count = 0;
    for(int i = 0; i<DICT_SIZE; i++){
        if(mystrcmp(A[(index+i)%DICT_SIZE].key, "!")) break;// if we get an empty space, then it should return NULL as that signifies the key is not present
        if(mystrcmp(A[(index+i)%DICT_SIZE].key,key)){// checks if we get the correct key
            tempArr[count] = A[(index+i)%DICT_SIZE];// stores the pointer of out entry in the ans
            count++;
        }    
    }
    ans = new struct Entry [count+1];
    for(int i = 0; i < count; i++){
        ans[i] = tempArr[i];
    }
    delete tempArr;
    strcpy(ans[count].key,"!");// indicates when the array ends
    return ans;
}