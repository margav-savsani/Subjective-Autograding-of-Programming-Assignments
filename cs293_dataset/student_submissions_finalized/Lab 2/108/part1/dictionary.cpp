...
int Dictionary::findFreeIndex(char key[]) {
    ...
    while (A[loc].value != 0 && A[loc].value != -1) { // Check for no element or tombstone
        loc++;
...
struct Entry* Dictionary::get(char key[]) {
    ...
    while ((A[loc].value)) {
        if (std::strcmp(key, A[loc].key) == 0 && A[loc].value != -1) return &A[loc];  // Match keys
        loc++;
        if (loc == N) loc -= N; // Circle around the array
