...
int Dictionary::hashValue(string key1){
	int x = 37;
	int m = DICT_SIZE;
	float A = 0.618;
	...
	for(int i = sz-1; i > 0; i--){
		//cout << key[i] << " " << key[i-1] << " ";
		calc = A*key1[i-1] + calc*key1[i] - int(A*key1[i-1] + calc*key1[i]);
		//cout << calc << endl;
	}
	return int(m*calc);
...
struct Entry *Dictionary::get(string my_key){
	int position = hashValue(my_key);
	int position_init = position;
	while(A[position].give_property() != 0){//while next position is not empty

		
		string stored_key1 = A[position].give_key();	
