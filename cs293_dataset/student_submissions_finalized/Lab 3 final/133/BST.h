...
class BST{
	...
	TreeNode *root;
        TreeNode *SENTINAL = new TreeNode;
...
public:
	...
	bool insert(int JourneyCode, int price);        
        ...
        struct TreeNode* find_node(int JourneyCode, int price);                     

	// Return true, if the ticket with given attributes is found, false otherwise
	bool find(int JourneyCode, int price);
	
        // Finds Succesor of the node
        struct TreeNode* find_succ(int JourneyCode, int price);
        
        //helps in removing an entry
        void remove_helper(TreeNode* temp, TreeNode* temp2);
	...
	// Methods for Part II
        ...
        int counter(TreeNode* root,int a, int b);
	...
	// Store the current tree in a file called filename.  You should
        ...
        void storing(TreeNode* root, ofstream &outfile);
	...
	// While loading a tree from a file written by customStore, we
        ...
        struct TreeNode* loader(ifstream &infile);
