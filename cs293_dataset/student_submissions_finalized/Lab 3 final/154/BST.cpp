...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    if(curr->price <= upperPriceBound && curr->price >= lowerPriceBound)
        count += 1;
...
void BST::customStore(string filename) { 
    ...
    TreeNode* curr = root;
    outfile<< curr->price << " " << curr->JourneyCode << "\n";
    outfile.close();
...
void BST::customLoad (int flag, string filename) {
    if(flag == LOAD_LEFT_SUBTREE){
        ...
        infile>>s2;
        // cout<<s1 << ", " << s2<<" inserted"<<endl;
        root = new TreeNode(stoi(s2), stoi(s1), nullptr, nullptr, nullptr);
        TreeNode* curr = root;
        if(!(infile >> s1)){
            delete root;
            root = nullptr;
        ...
        if(price > curr->price){
            ...
            curr->left = new TreeNode(stoi(s2), price, nullptr, nullptr, curr);
            curr = curr->left;
        }
        while(infile>>s1){
            if(s1 == "U"){
                // cout<< "U encountered"<<endl;
                curr =  curr->parent;
                if(curr == root) break;
                continue;
            }
            ...
            // cout<<s1 << ", " << s2<<" inserted"<<endl;
            if(price > curr->price){
                curr->right = new TreeNode(stoi(s2), price, nullptr, nullptr, curr);
                curr = curr->right;
            }
            else{
                curr->left = new TreeNode(stoi(s2), price, nullptr, nullptr, curr);
