...
void BST::prices(TreeNode* node, vector<int> &v){
    if(node==NULL)
        return;
    else {  
        //cout << node->JourneyCode << endl;
        v.push_back(node->price);
        prices(node->left, v);
        prices(node->right, v);
    }
}
...
void BST::preorderfile(TreeNode* node, ofstream &file){
    if(node==NULL)
        return;
    else {  
        string s1 = to_string(node->JourneyCode);
        string s2 = to_string(node->price);
        file << s1 << " " << s2 << endl;
        preorderfile(node->left, file);
        preorderfile(node->right, file);
    }
}
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    for(int i=0; i<v.size(); i++){
        if(v[i]<upperPriceBound && v[i]>=lowerPriceBound)
            count++;
    }
    return count;
}
