...
TreeNode* BST::findnode(int JourneyCode, int price){
    ...
    if(check->price == price){
        if((check->JourneyCode == JourneyCode)){
            // std::cout << "findnode " << check->JourneyCode << "\n";
            return check;
        }
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    int count = 0;
    if(root->price >= lowerPriceBound && root->price <= upperPriceBound) count++;
    count = count + BST(root->left).countJourneysInPriceBound(lowerPriceBound, upperPriceBound) + BST(root->right).countJourneysInPriceBound(lowerPriceBound, upperPriceBound);
    return count;
}
...
void BST::customStore(string filename) {
    ...
    output << root->JourneyCode << " " << root->price << "\n";
    output << "left\n"; BST(root->left).customStore(filename);
    output << "right\n"; BST(root->right).customStore(filename);
    return;
}
