...
struct TreeNode
{
        ...
        bool a, b; // denote if left and right are null or not , used for store and load
        // Constructors

        TreeNode() : price(0), JourneyCode(0), left(nullptr), right(nullptr), a(false), b(false) {}
        TreeNode(int jrnyCode, int jrnyPrice) : price(jrnyPrice), JourneyCode(jrnyCode), left(nullptr), right(nullptr), a(false), b(false) {}
        TreeNode(int jrnyCode, int jrnyPrice, TreeNode *leftChild, TreeNode *rightChild, TreeNode *parent) : price(jrnyPrice), JourneyCode(jrnyCode), left(leftChild), right(rightChild), parent(parent), a(false), b(false) {}
};
...
public:
        ...
        bool check(int JourneyCode, int price, TreeNode *x); //check if a journey code is present or not

        // Return true, if the ticket with given attributes is found, false otherwise
        bool find(int JourneyCode, int price); // finding a journey
        TreeNode *predecessor(TreeNode *z); // new function ,has its importance in delete
        void MYPRE_ORDER(TreeNode *y);// for traversals the below three functions are used
        void MYPOST_ORDER(TreeNode *y);
        void MYIN_ORDER(TreeNode *y);
        ...
        int countMore(int lowerPriceBound, int upperPriceBound, TreeNode *x); //counting recursively used in countJourneysInPiceBound
        ...
        void mystore(string filename, TreeNode *x); //storing in recursive manner
