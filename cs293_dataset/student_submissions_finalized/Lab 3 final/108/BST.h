...
public:
	...
	void traverse(int typeOfTraversal);
        ...
        // Traverses the tree in a dfs manner
        void dfs(TreeNode* node, int type, std::vector<TreeNode> &result);
	...
	// Methods for Part II
        ...
        // Finds the first element with price in the tree greater than value
        int search(const std::vector<TreeNode> &tree, int value);
	...
	// Returns the count of journeys that are at least as expensive
        ...
        // Returns the subtree size for node and updates the values in result
        int subtree_size(TreeNode *node, std::vector<std::vector<int>> &result);
	...
	// While loading a tree from a file written by customStore, we
        ...
        // It takes in a node and its position in the tree vector and 
        // creates its left and right node. Then it recursively calls 
        // itself with the newly created left and right nodes.
        void load_node (const std::vector<std::vector<int>> &tree, TreeNode* node, int pos);
};
