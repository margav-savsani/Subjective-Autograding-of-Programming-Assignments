...
struct TreeNode{
    ...
    // Constructors - changes: added parent(nullptr) in initialiser list where applicable
    ...
    TreeNode* succ_below(); //success node present in subtrees of given node
    void rem(); //removes node, helper for BST deletion
    bool copies(int jrnyCode); // returns if journey code is already present

    void store(ofstream& file); //helper session for custom store
...
public:
	...
	void customStore(string filename);
        // helper fuction for read, returns pointer to created node
        TreeNode* read(ifstream& file);	
