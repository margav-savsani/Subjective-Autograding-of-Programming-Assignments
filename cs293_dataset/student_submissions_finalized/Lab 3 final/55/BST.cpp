...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    BST left(root->left);
    int lcount=left.countJourneysInPriceBound(lowerPriceBound,upperPriceBound);
    if(root->price<=upperPriceBound && root->price>=lowerPriceBound){
        count+=1;
    };
    BST right(root->right);
    int rcount=right.countJourneysInPriceBound(lowerPriceBound,upperPriceBound);
...
void BST::customLoad (int flag, string filename) { 
    if(flag==LOAD_FULL_BST){
        ...
        file.open(filename);
        int jCode,price;
        while(file>>jCode>>price){
            TreeNode *node;
            node = new TreeNode;
            node->JourneyCode=jCode;
            node->price=price;
            if(temp.empty()){
                root=node;
                temp.push(node);
            }
            else if(node->price<=temp.top()->price){
                temp.top()->left=node;
                node->parent=temp.top();
                temp.push(node);
            }
            else if (node->price>temp.top()->price){
                TreeNode *dummy;
                while(!(temp.empty()) && node->price>temp.top()->price){
                    dummy=temp.top();
                    temp.pop();
                }
                dummy->right=node;
