...
public:
	...
	vector<string> getResult();
        ...
        //returns true if node with given journey code is present else returns false 
        //need of this function is that find() function finds that is (JourneyCode,price)
        //is present in tree but what if someone tries to insert same journeycode with 
        //different prices that should not happen.so this function is required.
        bool isJcode(TreeNode *node,int Jcode);
	...
	bool find(int JourneyCode, int price);
        ...
        //returns node with given journeycode and price if found else returns NULL
        TreeNode* getNode(int journeyCode,int price);

        //returns successor of given node used in remove function.
	struct TreeNode* succesessor(TreeNode* node);

	// Returns false, if JourneyCode isn't present
	// Deletes the corresponding entry and returns True otherwise
	bool remove(int JourneyCode, int price);

        //prints inorder traversal of BST  when called in traversal
        void inorder(TreeNode* node);

        //print postorder traversal of BST when called in traversal
        void postorder(TreeNode* node);

        //prints preorder traversal of BST when called in traversal
	...
	// Methods for Part II
        ...
        //returns count of journies in given price bound
        //purpose of implementation of this function is to reccurse over all nodes
        //of tree which was difficult to do so in original countJoourneyInPriceBound() function
        int count(TreeNode* node,int lowerBound,int upperBound);
	// Returns the count of journeys that are at least as expensive
        // as lowerPriceBound and at most as expensive as upperPriceBound,
        // both bounds included.
        int countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound);

        //stores all data of node given to it in a file and reccurse on it's left
        //and right node and stores all data of tree
        void store(TreeNode* node,ofstream &file);
	...
	void customStore(string filename);
        ...
        //loads tree from file name given to it in nodes by reccurssion.
	
        void load(TreeNode* &node, ifstream &file);
