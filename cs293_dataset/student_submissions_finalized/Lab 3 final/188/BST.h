...
public:
	...
	// Function defined for finding node.
	TreeNode* getNode(int JourneyCode, int price);
        //Function defined for finding the successor
	...
	void post_order(TreeNode* tnode);
	void pre_order(TreeNode* tnode);
	void in_order(TreeNode* tnode);
        void traverse(int typeOfTraversal);

        // int* BST::in_order_price(TreeNode *node);
	...
	int count_recursion(TreeNode* tnode, int lowerPriceBound, int upperPriceBound);
	...
	void pre_order_store(TreeNode* root, string filename);
