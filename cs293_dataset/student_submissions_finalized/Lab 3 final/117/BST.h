...
public:
	...
	// Inserts the element and returns True otherwise
        //Same cost is inserted in the right branch
	bool insert(int JourneyCode, int price);//done
        bool insertRecursive(int JourneyCode, int price, TreeNode* myroot);

	// Return true, if the ticket with given attributes is found, false otherwise
	bool find(int JourneyCode, int price);
        bool findRecursive(int JourneyCode, int price,TreeNode* myroot);
	
	// Returns false, if JourneyCode isn't present
	// Deletes the corresponding entry and returns True otherwise
	bool remove(int JourneyCode, int price);
        bool removeRecursive(int JourneyCode, int price, TreeNode* myroot, bool isleft);
        void deleteNode(TreeNode* node, bool isleft);
	...
	void traverse(int typeOfTraversal);
        void pretrav(TreeNode* myroot);
        void posttrav(TreeNode* myroot);
        void intrav(TreeNode* myroot);
        

	// Returns the price of the cheapest journey
	int getMinimum();
        TreeNode* getMinimumNode(TreeNode* myroot);
	...
	// Returns the count of journeys that are at least as expensive
        ...
        int recurintravPricecount(TreeNode* root,int lowerPriceBound, int upperPriceBound);
	...
	void customStore(string filename);
        void customStorerecur(TreeNode* myroot,const string& sentinal, ofstream& file);
