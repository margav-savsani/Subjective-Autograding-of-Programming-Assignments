...
public:
  ...
  // Return true, if JourneyCode is already present, false otherwise
  bool findJourneyCode(int JourneyCode);

  // Returns pointer to TreeNode having corresponding JourneyCode and Price
  TreeNode* getTreeNode(int JourneyCode, int price);

  // Returns node with minimum price in the BST whose root is passed
  TreeNode* getMinimumNode(TreeNode* curr);
  ...
  // Helper to countJourneysInPriceBound, does the complete job and returns total
  void helperCountJourneysInPriceBound(int& total, int lowerPriceBound, int upperPriceBound);
  ...
  void helperCustomStore(string filename);
