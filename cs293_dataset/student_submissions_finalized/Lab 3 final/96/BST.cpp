...
TreeNode* BST::search(TreeNode* node,int JourneyCode){
    if(node==nullptr) return nullptr;
    if(node->JourneyCode == JourneyCode) return node;
    if(search(node->left,JourneyCode)!=nullptr) return search(node->left,JourneyCode);
    else return search(node->right,JourneyCode);
}
...
bool BST::insert(int JourneyCode, int price) { 
    if(BST::search(root,JourneyCode)!=nullptr) return false;
    else{
...
bool BST::find(int JourneyCode, int price) {
    if(BST::search(root,JourneyCode)!=nullptr && BST::search(root,JourneyCode)->price==price) return true;
...
bool BST::remove(int JourneyCode, int price) {
    if(BST::search(root,JourneyCode)==nullptr) return false;
        ...
        root=BST::del(root,JourneyCode,price);
        return true;
    }
}
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    if(root->price >= lowerPriceBound && root->price <= upperPriceBound){
        count+=1+BST(root->left).countJourneysInPriceBound(lowerPriceBound,upperPriceBound) +BST(root->right).countJourneysInPriceBound(lowerPriceBound,upperPriceBound);
    }
    else if (root->price < lowerPriceBound) count+= BST(root->right).countJourneysInPriceBound(lowerPriceBound,upperPriceBound);
    else{
        count+=BST(root->left).countJourneysInPriceBound(lowerPriceBound,upperPriceBound);
    }
    return count;
}
...
void BST::customStore(string filename) {
    ...
    fprintf(fp, "%d ", root->JourneyCode);
    BST(root->left).customStore(fname);
    BST(root->right).customStore(fname);
}
