...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound, TreeNode *node_ptr)
{
    if (node_ptr == nullptr)
        return 0;
    if (node_ptr->price > upperPriceBound)
    {
        return countJourneysInPriceBound(lowerPriceBound, upperPriceBound, node_ptr->left);
    }

    if (node_ptr->price < lowerPriceBound)
    {
        return countJourneysInPriceBound(lowerPriceBound, upperPriceBound, node_ptr->right);
    }

    return 1 + countJourneysInPriceBound(lowerPriceBound, upperPriceBound, node_ptr->left) + countJourneysInPriceBound(lowerPriceBound, upperPriceBound, node_ptr->right);
}
...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) { return countJourneysInPriceBound(lowerPriceBound, upperPriceBound, root); }
...
void BST::customStore(string filename)
{
    ...
    customStore(outfile, root);
    outfile.close();
}

void BST::customStore(ofstream &outfile, TreeNode *node_ptr)
{
    outfile << node_ptr->JourneyCode << " " << node_ptr->price << std::endl;
    ...
    {
        customStore(outfile, node_ptr->left);
    }
    ...
    {
        customStore(outfile, node_ptr->right);
    }
}
    ...
    case LOAD_FULL_BST:
    {
        ...
        while (infile >> input)
        {
            if (input > 0)
            {
                ...
                if (direction == LEFT)
                {
                    current_node->left = new TreeNode(journeyCode, price, nullptr, nullptr, current_node);
                    current_node = current_node->left;
                }

                else
                {
                    current_node->right = new TreeNode(journeyCode, price, nullptr, nullptr, current_node);
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (true)
                    {
                        if (current_node->parent == nullptr && current_node->right != nullptr)
                        {
                            return;
                        }
