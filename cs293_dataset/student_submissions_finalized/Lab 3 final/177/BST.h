...
struct StorageUnit{
        ...
        StorageUnit() : JourneyCode(0), price(0){}
        StorageUnit(int JC,int pr) : JourneyCode(JC), price(pr){}
};
...
class Stack{
        ...
        Stack(int capa){head = -1; capacity = capa; arr = new TreeNode*[capa*sizeof(TreeNode*)];}

        bool isEmpty() {return (head == -1);}
        bool isFull() {return (head == capacity);}

        bool push(TreeNode* elem) {
                if(!isFull()) {
                        arr[++head] = elem;
                        return true;
                }
        ...
        TreeNode* pop() {if(isEmpty()) return nullptr; else return arr[head--];}
        TreeNode* top() {if(isEmpty()) return nullptr; else return arr[head];}
};
