...
public:
	...
	BST(TreeNode *root);
        ...
        ifstream infile;
        int Jcode, Jprice;
        int r = 0;  //used to store root pointer for using in loading of only left sub tree. 
        int val;
	...
	int getMinimum();
        ...
        void posto(TreeNode *tree);
        void ino(TreeNode *tree);
        int preo_count(TreeNode *tr, int lower, int upper);
        void readf(TreeNode *tre, string fil);
        void writef(TreeNode *tre, string fil, int flags);
