...
public:
	...
	bool insert(int JourneyCode, int price);
        TreeNode TreeSuccessor(TreeNode *m);
        TreeNode TreeMinimum(TreeNode *n);


	// Return true, if the ticket with given attributes is found, false otherwise
	bool find(int JourneyCode, int price);
        TreeNode find1(int JourneyCode, int price,TreeNode *n);
	...
	void traverse(int typeOfTraversal);
        void preorder(TreeNode *m);
        void postorder(TreeNode *m);
        void inorder(TreeNode *m);
