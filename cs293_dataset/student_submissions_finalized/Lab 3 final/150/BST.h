...
struct TreeNode{
    ...
    int getcount(int lhs , int rhs){
        int count =0;
        if (left!=NULL && price > lhs) {count = count + this->left->getcount(lhs,rhs);}
        if (right!=NULL && price <= rhs) {count = count + this->right->getcount(lhs,rhs);}
        if ( lhs <= price && price <= rhs){count++;}
        return count;
    }
};
...
public:
	...
	void myprint ( TreeNode*a , int i);
        void myout( TreeNode*a , ofstream& outfile );
        TreeNode* myload( TreeNode*a , ifstream& input);
