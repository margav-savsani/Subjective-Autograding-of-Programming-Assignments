...
struct TreeNode{
    ...
    bool same_node(TreeNode* n){
        if((n->price==this->price) && (n->JourneyCode==this->JourneyCode) && (n->left==this->left) &&(n->right==this->right) && (n->parent==this->parent))return 1;
        else return 0;
    }
};
...
public:
	...
	BST();
        BST(TreeNode* x);
	TreeNode* get_pointer_to_node(int JourneyCode,int price);
	void Pre_order(TreeNode* n);
	void Post_order(TreeNode* n);
	void In_order(TreeNode* n);
        int getMinimum();
        TreeNode* get_Successor(TreeNode* n);
        TreeNode* get_Minimum(TreeNode* n);
        TreeNode* get_leftmost(TreeNode* n);
