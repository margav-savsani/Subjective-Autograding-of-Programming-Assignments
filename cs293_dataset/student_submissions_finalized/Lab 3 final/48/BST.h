...
public:
	...
	int getMinimum();
        void treetoarray(TreeNode* node,int a[],int b[],int *i){
                if(node==nullptr) return;
                b[*i]=node->JourneyCode;
                a[*i]=node->price;
                ++*i;
                treetoarray(node->left,a,b,i);
                treetoarray(node->right,a,b,i);
        }
        TreeNode* arraytotree(int a[],int b[],int root,int max){
                if(root>max) return nullptr;
                TreeNode* d=new TreeNode(b[root],a[root]);
                int i=root;
                for (i = root; i <= max; i++){
                        if (a[i] > d->price) {
                        break;
                ...
                d->left=arraytotree(a,b,root+1,i-1);
                d->right=arraytotree(a,b,i,max);
                return d;
        }
