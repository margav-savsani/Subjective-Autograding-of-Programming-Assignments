...
int BST :: count(TreeNode* X, int a, int b){
  if(X==NULL) return 0;	
  if( (X->price == a) && (X->price == b) ) return 1;
  if((X->price >= a) && (X->price <= b))
      return 1+count(X->left, a, b)+count(X->right, a, b);
  else if(X->price < a)
      return count(X->right,a,b);    	 
  else return count(X->left,a,b);
}

int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
  TreeNode* X=root;
  int a = lowerPriceBound;
  int b = upperPriceBound; 
  return count(X, a, b);
}
...
void BST :: store(TreeNode* Y,string filename){
  ...
  file.open(filename);
     ...
     if(file.eof()) return;
        ...
        {
           file<< X->JourneyCode <<endl;
           file<< X->price <<endl;
           file.close();
           store(X->left,filename);
           store(X->right,filename);   
         } 
}
