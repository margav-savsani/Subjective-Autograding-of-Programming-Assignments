...
class BST{
	...
	TreeNode *root;
        ...
        // pointer to the found out Node after a "find" operation
        TreeNode *foundNode = NULL;
        
        // sub-functions for traversing the BST
        void preOrder(struct TreeNode* probe);
        void postOrder(struct TreeNode* probe);
        void inOrder(struct TreeNode* probe);

        // sub-function and variables to count the journeys, given a price range
        void inOrder_cmp(struct TreeNode* probe);
        int up_bound;
        int lower_bound;
        int count_cmp=0;

        struct TreeNode* successor(struct TreeNode* node1);
