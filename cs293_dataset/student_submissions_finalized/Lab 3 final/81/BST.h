...
public:
	...
	void printBST(const string& prefix, bool isLeft);
        // a function which returns the Succesor of a given node
	...
	void traverse(int typeOfTraversal);
        // just a dummy function for recursion to go on called inside transverse
        void traverse_dummy(int typeOfTraversal,TreeNode* root);
	...
	// Returns the count of journeys that are at least as expensive
        ...
        void Counter(TreeNode* node,int k1,int k2,int* count);
	...
	// Store the current tree in a file called filename.  You should
        ...
        // memory and time efficient as possible.
         void populateNodesInArray(TreeNode* r, int* arr,int* arr1, int pos);
        void treeToArray(TreeNode* node,int* arr,int* arr1,int maxNodes);
        void populateTreeFromArray(TreeNode* r, int* arr, int* arr1,int n, int pos);
        TreeNode* arrayToTree(int* arr, int n ,int* arr1);
