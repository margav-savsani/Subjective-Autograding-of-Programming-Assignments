...
int count_recursive(TreeNode *t, int min, int max)
{       
        int count_each_node=0;
        if (t != nullptr)
        {
                if (t->price == min){
                        count_each_node++;
                        count_each_node = count_each_node + count_recursive(t->right,min,max);
                }
                else if(t->price<min){
                        count_each_node = count_each_node + count_recursive(t->right,min,max);
                }
                        ...
                        count_each_node++;
                        count_each_node = count_each_node + count_recursive(t->left,min,max);
                }
                else if(t->price>max){
                        count_each_node = count_each_node + count_recursive(t->left,min,max);
                }
