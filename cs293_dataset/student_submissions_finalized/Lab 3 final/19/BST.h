...
struct TreeNode{
    ...
    int count(int lowerPriceBound,  int upperPriceBound){
        int c = 0;
        if(left != NULL){
                c += left.count(lowerPriceBound, upperPriceBound);
        }
        if(right != NULL ){
                c += right.count(lowerPriceBound, upperPriceBound);
        }
        if(price <= upperPriceBound && price >= lowerPriceBound){
                c ++;
        }
        return c;
    }
...
public:
	...
	bool remove(int JourneyCode, int price);
        ...
        void preorder(struct TreeNode* v){
                if(v == NULL){
                        return;
                }
                else{
        ...
        void postorder(struct TreeNode* v){
                if(v == NULL){
                        return; 
                }
                else{
        ...
        void inorder(struct TreeNode* v){
                if(v == NULL){
                        return;
                }
                else{
        ...
        struct TreeNode* deleteNode(struct TreeNode* a, int JourneyCode, int price){
                if (a == NULL)
                        return a;

                if (price < a->price)
                        a->left = deleteNode(a->left, JourneyCode, price);
 
                else if (price > a->price)
                        a->right = deleteNode(a->right, JourneyCode, price);
                        ...
                        struct TreeNode* temp = minValueNode(a->right);
                        a->JourneyCode = temp->JourneyCode;
                        a->price = temp->price;
                        a->right = deleteNode(a->right, temp->JourneyCode, temp->price);
                }
        return a;
        }
