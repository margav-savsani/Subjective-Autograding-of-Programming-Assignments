...
const int LOAD_FULL_BST=0;//these two have not been defined anywhere, so I defined them here
const int LOAD_LEFT_SUBTREE=1;
...
public:
	...
	bool insertelement(int JourneyCode, int price, TreeNode *root);


	// Return true, if the ticket with given attributes is found, false otherwise
	bool find(int JourneyCode, int price);
	bool findjourney(int JourneyCode, int price, TreeNode *root);
	...
	void traversal(int typeOfTraversal, TreeNode *root);
	...
	// Returns the count of journeys that are at least as expensive
        ...
        int countJourneysInPrice(int lowerPriceBound, int upperPriceBound, TreeNode *root);
	...
	void preordertraversal(TreeNode *root, string filename);
