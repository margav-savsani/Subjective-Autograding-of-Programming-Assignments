...
public:
        ...
        // Inserts are so that right subtree has values >= root, and left subtree is < root
        bool insert(int JourneyCode, int price);

        // Same specifications as insert, but in a given subtree only
        bool myInsert(int JourneyCode, int price, TreeNode *insertInMySubtree);

        // returns true iff that jouney code is presesnt on some ticket in the given subtree
        bool journeyExists(TreeNode *searchMe, int JourneyCode);

        // Return true, if the ticket with given attributes is found, false otherwise
        bool find(int JourneyCode, int price);

        // find the ticket with given attributes in the subtree rooted at an internal node.
        TreeNode *myFind(int JourneyCode, int price, TreeNode *searchMySubtree);
        ...
        // same specs as traverse but only in a subtree
        void myTraverse(int typeOfTraversal, TreeNode *traverseMe);
        ...
        // Returns the count of journeys in given subtree that are at least as expensive
        // as lowerPriceBound and at most as expensive as upperPriceBound,
        // both bounds included.
        int myCount(int lowerPriceBound, int upperPriceBound, TreeNode *searchMe);
        ...
        // in a manner such that your implementation of customLoad is as
        // memory and time efficient as possible.
        void myCustomStore(ofstream &outfile);
        ...
        // sub-tree, customLoad's time and space complexity should
        // be small even though the entire BST may be very large.
        void myCustomLoad(ifstream &infile, vector<TreeNode *> pendingNodes, vector<bool> insertOnLeft, int flag);
};
