...
// myfun returns -1 if direct journey  is not there to destination else it returns jrnycode of train
int myfunc( int jrnycode , listOfObjects<StationConnectionInfo *> *tostns , int destvalue , StationAdjacencyList adjacency[DICT_SIZE]){
  
  listOfObjects<StationConnectionInfo *> *t = tostns;
  ...
  if((t->object)->adjacentStnIndex == destvalue){
    return jrnycode;
  }
  //continuing with recursion 
  else{  
    listOfObjects<StationConnectionInfo *> *new_tostns = adjacency[(t->object)->adjacentStnIndex].toStations;
    return myfunc(jrnycode , new_tostns, destvalue , adjacency);
  }
}
