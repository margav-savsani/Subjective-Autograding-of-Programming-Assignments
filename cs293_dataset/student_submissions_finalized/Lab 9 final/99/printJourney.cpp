...
// used to store index of the station and all the trains associated with the station
class Stations
{
    public:
        int index;
        listOfObjects<TrainInfoPerStation *>* trains;
        Stations(int Index, listOfObjects<TrainInfoPerStation*>* Trains)
        {
            index = Index;
            trains = Trains;
        }
        // returns true if the train with the given journey code is among the trains of this station
        bool isPresent(int jrnyCode)
        {
            listOfObjects<TrainInfoPerStation *> * temp = trains;
            while (temp != nullptr)
            {
                if (temp->object->journeyCode == jrnyCode) return true;
                temp = temp->next;
            }
            return false;
        }
...
{
    // get the indices of the source and destination stations
