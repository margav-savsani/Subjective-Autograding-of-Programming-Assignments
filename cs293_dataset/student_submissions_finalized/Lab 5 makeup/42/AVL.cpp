...
bool AVLTree::rotateRightLeft(TreeNode *currNode, TreeNode *parentNode, TreeNode *grandParentNode)
{
  ...
  // currNode (x in Prof. Garg's lectures)
  // The last parameter is "false" to indicate that grandParentNode
  // should be made the right child of currNode
...
int AVLTree::imbal(TreeNode* node){
  ...
  else if(node->left == nullptr) return 1+node->right->height; 
  else if(node->right == nullptr) return 1+node->left->height;
  else return abs(node->left->height - node->right->height); 
}

void AVLTree::choose_rotate(TreeNode *&x, TreeNode *&y, TreeNode*&z){
    bool left1 = (z->left == y);
    bool left2 = (y->left == x);
    if (left1 && left2){
...
bool AVLTree::insert(int jrnyCode, int price)
{
  ...
  // see fit.
    ...
    while (pos_ins != nullptr) {
        if (AVLLessThan(a,pos_ins)){
            prnt=pos_ins;
            pos_ins=pos_ins->left;
            left=true;
        }
        else if (AVLLessThan(pos_ins,a)){
            prnt=pos_ins;
            pos_ins=pos_ins->right;
            left=false;
        }
        else{
            if (price < pos_ins->get_price()){
                prnt=pos_ins;
                pos_ins=pos_ins->left;
                left=true;
            }
            else if (price == pos_ins->get_price()) return false;
            else{
                prnt=pos_ins;
                pos_ins=pos_ins->right;
                left=false;
    ...
    if (left){
        if (prnt->right == nullptr) prnt->height++;
        else return true;
    }
    else{
        if (prnt->left == nullptr) prnt->height++;
        else return true;
    }
    if (grpt == nullptr) 
    {
    ...
    while(grpt != nullptr && imbal(grpt) <= 1){
        if (imbal(grpt) == 0) return true;
        grpt->height++;
        pos_ins = prnt;
        prnt = grpt;
        grpt = grpt->parent;
    }
    if (grpt == nullptr) return true;
    if (imbal(grpt) == 2){
        choose_rotate(pos_ins,prnt,grpt);
    }
...
int AVLTree::find(int jrnyCode)
{
  ...
  while (pos_ins != nullptr){
      if (find_node < pos_ins) pos_ins = pos_ins->left;
      else if (pos_ins < find_node) pos_ins = pos_ins->right;
      else return pos_ins->get_price();
  }
  return -1;
}
