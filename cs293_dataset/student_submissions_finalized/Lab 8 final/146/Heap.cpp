  ...
  if(t->parent == NULL){
    ...
    if(t->parent == NULL){
      ...
      if(t->parent->right == NULL){
        t->parent->right = new TreeNode<T>(obj);
...
void Heap<T>::insert(T obj) {
  if(root==NULL){
    ...
    last = insertionNode;
    while(insertionNode->parent!=NULL && insertionNode->parent->object < insertionNode->object ){
...
void Heap<T>::delMax() {
  ...
  while(true){
    ...
    if(!(currNode->object < currNode->left->object ) && !(currNode->object < currNode->right->object )) break; //Should be less than equal to

    if(currNode->object < currNode->left->object ){
      custswap(currNode->left,currNode);
      currNode = currNode->left;
    }
    else if( currNode->object < currNode->right->object  ){
