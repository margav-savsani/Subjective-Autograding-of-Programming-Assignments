...
bool Trie::insert(string wordInName, int indexOfName) {
  ...
  for (int i = 0; i<len; i++){
    ...
    int curr_index = (int)wordInName[i] -97;
    ...
    if(i == len-1){
      if(currNode->isWord){
        ...
        while(list_to_insert_index!= nullptr){
          cout << "already found index " << list_to_insert_index->object << endl;
          list_to_insert_index = list_to_insert_index->next;
        }
        listOfObjects<int> list_obj(indexOfName);
        list_to_insert_index = &list_obj;
        cout << "inserted new object in list with index number: " << list_to_insert_index->object << endl;
