...
{
public:
  // the below two attributes account for the $ node as discussed in lecture
  bool isWord;
  listOfObjects<int> *indices; // Indices in stnNameToIndex dictionary;
                               // non-null only if isLeaf is true
                               // leaf ban na chahiye tha
  int numNonNullBranches;
  TrieNode *branch[NUM_CHARS]; // store pointer to further nodes alphabet-wise
...
private:
  ...
  TrieNode *root; // It is the root of the trie
