...
bool Trie::insert(string wordInName, int indexOfName)
{
  ...
  {
    bool letterFound;
    for (int i = 0; i < currNode->numNonNullBranches; i++)
    {
      if (currNode->branch[i]->letter == wordInName[j])
      {
        letterFound = true;
    ...
    else
    {
      ...
      node->letter = wordInName[j];
      currNode->branch[currNode->numNonNullBranches] = node;
  ...
  {
    if (currNode->indices->object == indexOfName)
    {
      nameFound = true;
      break;
    }
