...
bool Trie::insert(string wordInName, int indexOfName)
{
  ...
  for (int wordIterator = 0; wordIterator < len; wordIterator++)
  {
    ...
    int letterValue = letter - 'a'; // letter is converted into an integer in range [0, NUM_CHARS]
    if (root->branch[letterValue] == nullptr)
    {
    ...
    if (wordIterator == len - 1)
    {
      ...
      {
        root->branch[letterValue]->indices->next = new listOfObjects<int>(indexOfName);
      }
    }
