...
bool Trie::addCompletions(TrieNode *node, listOfObjects<string> *currCompletions) {
  ...
  if(node->isWord){
    ...
    while(curr != nullptr){
      if(to_add_at == nullptr) to_add_at = new listOfObjects<string>(nameDict->getKeyAtIndex(curr->object));
      else to_add_at->next = new listOfObjects<string>(nameDict->getKeyAtIndex(curr->object));
      to_add_at = to_add_at->next;
      curr = curr->next;
    }
    return true;
