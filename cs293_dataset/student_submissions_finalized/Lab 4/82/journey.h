...
public:
  ...
  void setParams(unsigned int JourneyCode, unsigned int price){
  ...
  // istream& operator >> (istream& in, Journey &j); // Usage: cin >> journey1;
  // ostream& operator << (ostream& out, const Journey& j); // Usage: cout << journey2
};
