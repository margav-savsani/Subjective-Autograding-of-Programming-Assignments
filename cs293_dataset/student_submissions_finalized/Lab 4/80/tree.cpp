...
int BST::countJourneysInPriceBound(int lowerPriceBound, int upperPriceBound) {
    ...
    if (root == nullptr){
        ...
        int number_left = 0, number_center = 0, number_right = 0;
        if(curr->price > lowerPriceBound){
            root = curr->left;
            number_left = countJourneysInPriceBound(lowerPriceBound, upperPriceBound);
        }

        if(curr->price >= lowerPriceBound && curr->price <= upperPriceBound){
            number_center = 1;
        }

        if(curr->price <= upperPriceBound){
            root = curr->right;
            number_right = countJourneysInPriceBound(lowerPriceBound, upperPriceBound);
        }
        root = curr;
        return number_left + number_center + number_right;
...
void BST::customLoad (int flag, string filename) { 
    ...
    if (flag == LOAD_FULL_BST){
        ...
        while (infile >> j_code){
            infile >> j_price;
            TreeNode *my_node = new TreeNode(j_code, j_price);
            ...
            while (mark!=0 && arr[mark-1]->price < my_node->price){
                mark-=1;
            }

            if (arr[mark]->price > my_node->price){
                arr[mark]->left = my_node;
                my_node->parent = arr[mark];
                mark+=1;
                arr[mark] = my_node;
            }
            else{
                arr[mark]->right = my_node;
                my_node->parent = arr[mark];
                arr[mark] = my_node;
            }   
        }
