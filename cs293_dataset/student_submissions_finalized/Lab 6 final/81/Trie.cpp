...
bool Trie::insert(string wordInName, int indexOfName) {
  ...
  for(int i=0;i<len;i++){
  TrieNode* next;
  next = currNode->branch[int(wordInName[i])-65];
  if(next==nullptr){
...
bool Trie::del(string wordInName, string name) {
  ...
  if(currNode->indices->next==nullptr){recursiveDelete(lastBranchNode);}
  else {
    listOfObjects<int>* node;
    listOfObjects<int>* prev;
    node=currNode->indices;
    while(node!=nullptr){
      if(nameDict->get(name)->value==node->object){
        prev->next=node->next;
        return true;
      }
      prev=node;
