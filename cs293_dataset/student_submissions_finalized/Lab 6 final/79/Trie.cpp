...
bool Trie::insert(string wordInName, int indexOfName) {
  ...
  if (isPresent(wordInName)){
    // if word is already present in the trie
    // just append the  indexOfName 
    for (int i = 0; i < len; i++){
...
bool Trie::del(string wordInName, string name) {
  ...
  int value = nameDict->get(name)->value;
  while(currNode->indices->next != nullptr){
    if(currNode->indices->object == value){
      break;
    }
    prevNode = currNode->indices;
    currNode->indices = currNode->indices->next;
  }
  if(prevNode == nullptr){
    currNode->indices = currNode->indices->next;
    ...
    if(currNode->indices->object == value){
      prevNode->next = currNode->indices->next;
    }
    prevNode->next = currNode->indices->next;
  }
...
bool Trie::addCompletions(TrieNode *node, listOfObjects<string> *currCompletions) {
  ...
  // Insert your code here
  while(node->indices->next != nullptr) {
    int index = node->indices->object;
    string stationName = nameDict->getKeyAtIndex(index);
    listOfObjects<string> *station = new listOfObjects<string>(stationName);
    if(currCompletions->object == ""){
      currCompletions->object = stationName;
    }
    else{
      while(currCompletions->next != nullptr){
        currCompletions = currCompletions->next;
      }
      currCompletions->next = station;
    }
