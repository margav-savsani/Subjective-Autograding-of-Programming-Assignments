...
class Trie {
 ...
 public:
  ...
  void updateindexOfName(listOfObjects<int> *indices, int indexOfName);
  void printTrie();
  void recursivePrintTrie(TrieNode *node, const string& prefix, int branchId, bool siblingsToRight);
  void deleteindexofName(listOfObjects<int> *indices, int indexOfName);
  void deletewordInName(string wordInName, listOfObjects<TrieNode*> *multi);
};
