...
int *Planner::computeHMatrixForKMP(string pattern) {
  ...
  for(int i = 1; i < pattern.length();i++){ //Computing the HM[i] using the previous computed
    if(toupper(pattern[i]) == toupper(pattern[lenofpattern])){ //IF the pattern matches again then we return HM[i-1] + 1 (which is lenofpattern+1)
      lenofpattern++;
...
int Planner::KMPMatch(string text, int *hMatrix, string pattern) {
  ...
  while(text.length() - indtxt >= pattern.length() - indpat){
    ...
    if(toupper(text[indtxt]) == toupper(pattern[indpat])){
      indtxt++;
      indpat++;
    }
    ...
    else if (indtxt < text.length() && toupper(pattern[indpat]) != toupper(text[indtxt])){ 
