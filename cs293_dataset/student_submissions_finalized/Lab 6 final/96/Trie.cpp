...
bool Trie::insert(string wordInName, int indexOfName) {
  ...
  if(currNode->indices == nullptr) currNode->indices = new listOfObjects<int>(indexOfName);
    ...
    while(indices1->next != nullptr) indices1=indices1->next;
  ...
  if(true){
    if(currNode->indices->next == nullptr){
      ...
      while(currNode->indices != nullptr){
        ...
        else if(nameDict->getKeyAtIndex(currNode->indices->next->object) == name){
...
listOfObjects<string> * Trie::completions(string prefix) {
  ...
  for(int i=0;i<len;i++){
    ...
    if(currNode->branch[index] == nullptr) return nullptr;
...
bool Trie::addCompletions(TrieNode *node, listOfObjects<string> *currCompletions) {
  ...
  if(node->isWord){
    if(c == nullptr){
      c= new listOfObjects<string>(nameDict->getKeyAtIndex(node->indices->object));
      ...
      while(node->indices != nullptr){
        if(!isPresentInCompletions(currCompletions, nameDict->getKeyAtIndex(node->indices->object))){
          c->next->object=nameDict->getKeyAtIndex(node->indices->object);
          c=c->next;
        }
        node->indices=node->indices->next;
      }
