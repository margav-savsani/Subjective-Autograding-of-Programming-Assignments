...
class TrieNode {
 public:
  bool isWord; //true if the node is end of a word 
  listOfObjects<int> *indices; // Inull if no word ends there , else contains the words that end here .
  int numNonNullBranches; //count of the number of branches 
  TrieNode *branch[NUM_CHARS]; // array of trie nodes .
...
class Trie {
 ...
 public:
  ...
  bool del(string wordInName, string fullWord);
