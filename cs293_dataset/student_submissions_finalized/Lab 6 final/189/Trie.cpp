...
bool Trie::del(string wordInName, string name) {
  ...
  if(currNode->indices==nullptr) return false;
  
  else{
    if(this->nameDict->getKeyAtIndex(currNode->indices->object)==name){
      currNode->indices=currNode->indices->next;
      return true;
    }
    else{
      temp=currNode->indices->next;
      while (this->nameDict->getKeyAtIndex(temp->object)!=name&&temp!=nullptr)
      {
        temp=temp->next;
        prev=prev->next;
      }
