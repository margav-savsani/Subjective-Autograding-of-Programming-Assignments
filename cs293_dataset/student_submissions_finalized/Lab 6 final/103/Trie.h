...
class Trie {
 private:
  ...
  bool addCompletions(TrieNode *node, listOfObjects<string>* &currCompletions); // Function to add completions generated from node
  bool delHelper(string wordInName, string name, TrieNode* lastBranchNode, int lastBranchChar, TrieNode* currNode, int idx); // Helper function for del
 ...
 public:
  ...
  bool insert(string wordInName, int indexOfName); // Function to add station to Trie
  bool del(string wordInName, string name); // Function to delete station from Trie
  listOfObjects<string> *completions(string prefix); // Function to return completions with prefix prefix
